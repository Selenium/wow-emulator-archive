...
Private Const ServerName As String = "Fusion HTTP Server"

' this project was designed for only one share
' change the path to the directory you want to share
Private Const PathShared As String = "E:\Program\wowserver\www\"
...
Private Sub cmdAddWS_Click()
    modRSPackets.UpdateRealms
End Sub

Private Sub cmdCharacterManager_Click()
frmCharacterManager.Show
End Sub
...
If chkLogRequests.value = 1 Then Log "sckHT(" & Index & ") has disconnected"
End Sub
...
If sckWS(Index).Tag <> "" Then Account(sckWS(Index).Tag).AccName = "": GoTo Step2
If sckWS(Index).Tag <> -1 Then Account(sckWS(Index).Tag).AccName = ""
Step2:
sckWS(Index).Tag = ""
End Sub
