...
{
	std::list<DOTCONFDocumentNode*>::iterator j;
	std::list<char*>::iterator i;
    for(j = nodeTree.begin(); j != nodeTree.end(); j++){
        delete(*j);
    }
    for(i = requiredOptions.begin(); i != requiredOptions.end(); i++){
        free(*i);
    }
    for(i = processedFiles.begin(); i != processedFiles.end(); i++){
        free(*i);
    }
    free(fileName);
    delete mempool;
    ...
    if(!ret){
		...
		for(std::list<DOTCONFDocumentNode*>::iterator i = nodeTree.begin(); i!=nodeTree.end(); i++){
            ...
            if(!cmp_func("Include", tagNode->name)){
                vi = 0;
                while( vi < tagNode->valuesCount ){
                    if(access(tagNode->values[vi], 04) == -1){
