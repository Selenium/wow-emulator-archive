...
//it is inserted into the list with other connections
Socket *Network::CreateListener (int port)
{
	...
	//note that 32K can be read from 1 go at a time
	opt = 32767;//32K aprox many very big packets as buffer to minimize the chance that internal buffer will be required
	setsockopt (sh, SOL_SOCKET, SO_RCVBUF, (char *)&opt, sizeof (opt));
	setsockopt (sh, SOL_SOCKET, SO_SNDBUF, (char *)&opt, sizeof (opt));
