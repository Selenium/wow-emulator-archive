...
void WorldSession::HandleItemTextQuery(WorldPacket & recv_data )
{
	...
	Log::getSingleton().outDebug("Mail %d found. Info: %d",mailguid,unk1);
	...
	if(!itr) {
		...
		Log::getSingleton().outDebug("Mailerror %d",mailguid);
}
...
void WorldSession::HandleMailCreateTextItem(WorldPacket	& recv_data	)
{
	...
	Log::getSingleton().outString("Mail:: CreateTextItem unk1=%d,unk2=%d,mailid=%d",unk1,unk2,mailid);
