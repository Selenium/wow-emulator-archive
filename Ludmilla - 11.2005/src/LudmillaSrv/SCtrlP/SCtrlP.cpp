...
{
	DWORD dwWritten;
	ch(WriteFile(LudaStdIn, buf, size, &dwWritten, NULL));
}
//------------------------------------------------------------------------------
							// Защищённые методы:
//------------------------------------------------------------------------------
...
void CSCtrlP::Open(char *host)
{
	...
	__try
	{
		...
		LudaStdOut = INVALID_HANDLE_VALUE;
		LudaStdIn = INVALID_HANDLE_VALUE;
		ch(WaitNamedPipe(stdinname, NMPWAIT_USE_DEFAULT_WAIT));
		chf(INVALID_HANDLE_VALUE, LudaStdIn = CreateFile(stdinname,
			GENERIC_WRITE, 
			0,              // no sharing
		...
		chf(INVALID_HANDLE_VALUE, LudaStdOut = CreateFile(stdoutname,
			GENERIC_READ, 
			0,              // no sharing
			NULL,           // default security attributes
			OPEN_EXISTING,  // opens existing pipe
...
void  CSCtrlP::ThreadProc()
{
	...
	__try
	{
		...
		for(;;)
		{
			...
			// Reading from child:
			while (ARead(LudaStdOut, bufChild, sizeof(bufChild), &cbReadChild, &ol, true))
			...
			switch(WaitForMultipleObjects(2, wait, FALSE, INFINITE))
			{
				...
				case WAIT_OBJECT_0+1:
					SResult(LudaStdOut, &ol, &cbReadChild, true);
					SWrite(StdOut, bufChild, cbReadChild, false, false);
					break;
				default:
