...
void CRunner::Start(HANDLE stopevent)
{
	...
	__finally
	{
		...
		TryClose(hLudaStdOutW);
		TryClose(hLudaStdInR);
...
CRunner::CRunner()
{
	...
	hLudaStdOutW = hLudaStdInR = INVALID_HANDLE_VALUE;
...
void CRunner::CreatePublicPipes()
{
	...
	__try
	{
		...
		// Create named pipes:
		chf(INVALID_HANDLE_VALUE, hLudaStdOutW = CreateNamedPipe(outname,
		...
		chf(INVALID_HANDLE_VALUE, hLudaStdInR = CreateNamedPipe(inname,
...
newclient:
	...
	AConnect(hLudaStdInR, &ol_Client);
clearechild: // without clint we need not data from child
	...
	if (!ConnectNamedPipe(hLudaStdOutW, NULL))
	{
		DWORD e = GetLastError();
	...
	{
		// Reading from client:
		while (ARead(hLudaStdInR, bufClient, sizeof(bufClient), &cbReadClient, &ol_Client))
			if (WaitForSingleObject(stopevent, 0) == WAIT_TIMEOUT)
		...
		while (ARead(hChildStdOutR, bufChild, sizeof(bufChild), &cbReadChild, &ol_Child, true))
			if (WaitForSingleObject(stopevent, 0) == WAIT_TIMEOUT)
				SWrite(hLudaStdOutW, bufChild, cbReadChild);
			else
				return;
		...
		switch (WaitForMultipleObjects(4, wait, FALSE, INFINITE))
		{
			...
			case WAIT_OBJECT_0+2: // client have written data
				...
				SResult(hLudaStdInR, &ol_Client, &cbReadClient);
				SWrite(hChildStdInW, bufClient, cbReadClient, true, false);
	...
	catch(ErrEvents err)
	{
		if (err == PipeIsClosed)
		{
			ch(DisconnectNamedPipe(hLudaStdInR));
			ch(DisconnectNamedPipe(hLudaStdOutW));
			goto newclient;
		}
	}
	throwe("never here");
