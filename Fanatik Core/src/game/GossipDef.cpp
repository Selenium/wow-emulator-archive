...
{
    WorldPacket data( SMSG_GOSSIP_MESSAGE, (120) );         // guess size
    data << npcGUID;
...
void PlayerMenu::SendQuestGiverQuestList( QEmote eEmote, std::string Title, uint64 npcGUID )
{
    ...
    for ( uint16 iI = 0; iI < pQuestMenu->MenuItemCount(); iI++ )
    {
        ...
        data << questID;
        data << uint32( qmi.m_qIcon ); //2.0.6 ???? why
        data << uint32( objmgr.QuestTemplates[questID]->GetQuestLevel() );
        data << objmgr.QuestTemplates[questID]->GetTitle();
    }
    pSession->SendPacket( &data );
...
{
    WorldPacket data(SMSG_QUESTGIVER_QUEST_DETAILS, 120);   // guess size 2.0.6
...
{
    WorldPacket data( SMSG_QUEST_QUERY_RESPONSE, 120 );     // guess size 2.0.6
    ...
    for (iI = 0; iI < QUEST_REWARDS_COUNT; iI++)
    {
        ...
        data << uint32(0);   //temp hack for tbc 2.0.6
    data << uint32(0);   //temp hack for tbc 2.0.6
...
void PlayerMenu::SendQuestGiverOfferReward( uint32 quest_id, uint64 npcGUID, bool EnbleNext, QEmote Emotes[], unsigned int EmoteCnt )
{
    ...
    WorldPacket data( SMSG_QUESTGIVER_OFFER_REWARD, 62 );   // guess size 2.0.6
...
void PlayerMenu::SendQuestGiverRequestItems( Quest *pQuest, uint64 npcGUID, bool Completable, bool CloseOnCancel )
{
    ...
    WorldPacket data( SMSG_QUESTGIVER_REQUEST_ITEMS, 62 );  // guess size 2.0.6
    data << npcGUID;
    ...
    ItemPrototype const *pItem;
    for (int i = 0; i < pQuest->GetReqItemsCount(); i++) //2.0.6
    {
    ...
    if ( !Completable )
        data << uint32(0x00);
         ...
         data << uint32(0x08);//data << uint32(0x03);  test 2.0.3
    data << uint32(10);//data << uint32(0x04) << uint32(0x08) << uint32(0x10); test 2.0.3
