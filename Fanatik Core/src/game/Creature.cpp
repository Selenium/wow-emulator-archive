...
uint32 Creature::getDialogStatus(Player *pPlayer, uint32 defstatus)
{
	...
	uint32 result1 = DIALOG_STATUS_NONE;
    QuestStatus status;
    uint32 quest_id;
    Quest *pQuest;
    ...
    for( std::list<uint32>::iterator i = mInvolvedQuests.begin( ); i != mInvolvedQuests.end( ); i++ )
    {
            ...
            (pQuest->IsAutoComplete() && pPlayer->CanTakeQuest(pQuest, false)))
        {
            ...
            result1 = DIALOG_STATUS_INCOMPLETE;
    }
	...
	if (result1 == DIALOG_STATUS_INCOMPLETE)
		return result1;
	else if (result == DIALOG_STATUS_UNAVAILABLE)
		return result;
...
bool Creature::CreateFromProto(uint32 guidlow,uint32 Entry)
{
	...
	LoadCreaturesAddon(guidlow); //2.0.8
...
bool Creature::LoadCreaturesAddon(uint32 guidlow)
{
    ...
    QueryResult *result = sDatabase.PQuery("SELECT `id`,`modelid`,`mount`,`bytes0`,`bytes1`,`bytes2`,`emote`,`aura`,`auraflags`,`auralevels`,`auraapplications`,`aurastate` FROM `creature_addon` WHERE `guid` = '%u'", guidlow);
    if(!result)
	{
		return false;
	}
	...
	{
	SetUInt32Value(UNIT_FIELD_DISPLAYID, fields[1].GetUInt32());
    SetUInt32Value(UNIT_FIELD_NATIVEDISPLAYID, fields[1].GetUInt32() );
	}
	if (fields[2].GetUInt32() != 0)
        Mount(fields[2].GetUInt32());
    
	if (fields[3].GetUInt32() != 0)
	SetUInt32Value(UNIT_FIELD_BYTES_0, fields[3].GetUInt32());
    
	if (fields[4].GetUInt32() != 0)
	SetUInt32Value(UNIT_FIELD_BYTES_1, fields[4].GetUInt32());

	if (fields[5].GetUInt32() != 0)
	SetUInt32Value(UNIT_FIELD_BYTES_2, fields[5].GetUInt32());

    if (fields[6].GetUInt32() != 0)
	SetUInt32Value(UNIT_NPC_EMOTESTATE, fields[6].GetUInt32());

	if (fields[7].GetUInt32() != 0)
	SetUInt32Value(UNIT_FIELD_AURA, fields[7].GetUInt32());

	if (fields[8].GetUInt32() != 0)
	SetUInt32Value(UNIT_FIELD_AURAFLAGS, fields[8].GetUInt32());

	if (fields[9].GetUInt32() != 0)
	SetUInt32Value(UNIT_FIELD_AURALEVELS, fields[9].GetUInt32());

	if (fields[10].GetUInt32() != 0)
	SetUInt32Value(UNIT_FIELD_AURAAPPLICATIONS, fields[10].GetUInt32());
  
	if (fields[11].GetUInt32() != 0)
	SetUInt32Value(UNIT_FIELD_AURASTATE, fields[11].GetUInt32());
