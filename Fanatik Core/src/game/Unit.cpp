...
void Unit::RemoveAllAurasOnDeath()
{
    ...
    {
		//ADD FROM CODEBREAKER TO CREATE FLAG ON LOCATION OF THE DEAD PLAYER
		if(iter->second->GetId()==23335||iter->second->GetId()==23333)
		{
		//player has a flag warsong or silverwing
			if(iter->second->GetId()==23335){
				team=ALLIANCE;
			}
			sLog.outError("le gars a le spell !!");
			sLog.outError("Le spell a ete suppr avec succès x:%f,y:%f,z:%f",GetPositionX(),GetPositionY(),GetPositionZ());
			if(!BattleGround::SpawnFlagOnPlayerDeath(team,GetPositionX(),GetPositionY(),GetPositionZ()))
			{
				sLog.outError("pas marché :s:s");
			}
		}
		//FINISH HERE
...
void Unit::AddItemEnchant(Item *item,uint32 enchant_id,uint8 enchant_slot,bool apply)
{
    ...
    SetUInt32Value(VisibleBase+1 + enchant_slot, apply? item->GetUInt32Value(ITEM_FIELD_ENCHANTMENT+enchant_slot*4) : 0);
}
