...
{
    QueryResult *result = sDatabase.Query("SELECT `entry`, `name`, `period`, `offset` FROM transports");
...
void Transport::Update(uint32 p_time) {
    ...
    m_timer = (getMSTime() - m_offset) % m_period;
...
//        MapManager::Instance().GetMap(m_curr->second.mapid)->Add(&this); // -> // ->Add(t);        
        ...
        if ((sLog.getLogFilter() & LOG_FILTER_TRANSPORT_MOVES)==0)
			sLog.outDetail("%s moved to %f %f %f [%i]", this->m_name.c_str(), m_curr->second.x, m_curr->second.y, m_curr->second.z, m_curr->second.mapid);
