...
void Spell::prepare(SpellCastTargets * targets)
{
    ...
    if(IsAutoRepeat() && (m_spellInfo->AttributesEx2 != 0x000020))
...
void Spell::SendSpellCooldown()
{
    ...
    //_player->GetSession()->SendPacket(&data); //2.0.6
...
void Spell::SendSpellGo()
{
    ...
    */
	...
	//temp heck 2.0.7 for flying mount
	if(m_spellInfo->Id ==32345)
	{
		        
		if(m_caster->IsFlymount()==false)
		{
			m_caster->SetUInt32Value(147, 17890);
			WorldPacket data2(835, (20));  
			data2 << uint8(0xFF) << m_caster->GetGUID();
	        data2 << uint32(2);
            m_caster->SendMessageToSet(&data2,true);
			m_caster->SetFlymount(true);

			data.Initialize(897);               //a9
			data << uint8(0xFF) << m_caster->GetGUID();
			data << (uint32)0;
			data << (float)40; //defualt speed - no command needed
			m_caster->SendMessageToSet(&data,true);
		}
	
	}
	// Windrider Mounts -- NonEpic
	if(m_spellInfo->Id ==32243)	// Tawny
	{
		        
		if(m_caster->IsFlymount()==false)
		{
			m_caster->SetUInt32Value(147, 17699);
			WorldPacket data2(835, (20));  
			data2 << uint8(0xFF) << m_caster->GetGUID();
	        data2 << uint32(2);
            m_caster->SendMessageToSet(&data2,true);
			...
			data << uint8(0xFF) << m_caster->GetGUID();
			data << (uint32)0;
			data << (float)25; //defualt speed - no command needed
			m_caster->SendMessageToSet(&data,true);
		}
	
	}
	if(m_spellInfo->Id ==32244)	// Blue
	{
		        
		if(m_caster->IsFlymount()==false)
		{
			m_caster->SetUInt32Value(147, 17700);
			WorldPacket data2(835, (20));  
			data2 << uint8(0xFF) << m_caster->GetGUID();
	        data2 << uint32(2);
            m_caster->SendMessageToSet(&data2,true);
	...
	if(m_spellInfo->Id ==32245)	// Green
	{
		        
		if(m_caster->IsFlymount()==false)
		{
			m_caster->SetUInt32Value(147, 17701);
			WorldPacket data2(835, (20));  
			data2 << uint8(0xFF) << m_caster->GetGUID();
	        data2 << uint32(2);
            m_caster->SendMessageToSet(&data2,true);
	...
	// Windrider mounts - epic
	if(m_spellInfo->Id ==32246)	// Swift Red
	{
		        
		if(m_caster->IsFlymount()==false)
		{
			m_caster->SetUInt32Value(147, 17719);
			WorldPacket data2(835, (20));  
			data2 << uint8(0xFF) << m_caster->GetGUID();
	        data2 << uint32(2);
            m_caster->SendMessageToSet(&data2,true);
			...
			data << uint8(0xFF) << m_caster->GetGUID();
			data << (uint32)0;
			data << (float)35; //defualt speed - no command needed
			m_caster->SendMessageToSet(&data,true);
		}
	
	}
	if(m_spellInfo->Id ==32295)	// Swift Green
	{
		        
		if(m_caster->IsFlymount()==false)
		{
			m_caster->SetUInt32Value(147, 17720);
			WorldPacket data2(835, (20));  
			data2 << uint8(0xFF) << m_caster->GetGUID();
	        data2 << uint32(2);
            m_caster->SendMessageToSet(&data2,true);
	...
	if(m_spellInfo->Id ==32296)	// Swift Yellow
	{
		        
		if(m_caster->IsFlymount()==false)
		{
			m_caster->SetUInt32Value(147, 17722);
			WorldPacket data2(835, (20));  
			data2 << uint8(0xFF) << m_caster->GetGUID();
	        data2 << uint32(2);
            m_caster->SendMessageToSet(&data2,true);
	...
	if(m_spellInfo->Id ==32297)	// Swift Purple
	{
		        
		if(m_caster->IsFlymount()==false)
		{
			m_caster->SetUInt32Value(147, 17721);
			WorldPacket data2(835, (20));  
			data2 << uint8(0xFF) << m_caster->GetGUID();
	        data2 << uint32(2);
            m_caster->SendMessageToSet(&data2,true);
	...
	// Gryphon Mounts -- NonEpic
	if(m_spellInfo->Id ==32235)	// Golden
	{
		        
		if(m_caster->IsFlymount()==false)
		{
			m_caster->SetUInt32Value(147, 17697);
			WorldPacket data2(835, (20));  
			data2 << uint8(0xFF) << m_caster->GetGUID();
	        data2 << uint32(2);
            m_caster->SendMessageToSet(&data2,true);
	...
	if(m_spellInfo->Id ==32239)	// Ebon
	{
		        
		if(m_caster->IsFlymount()==false)
		{
			m_caster->SetUInt32Value(147, 17694);
			WorldPacket data2(835, (20));  
			data2 << uint8(0xFF) << m_caster->GetGUID();
	        data2 << uint32(2);
            m_caster->SendMessageToSet(&data2,true);
	...
	if(m_spellInfo->Id ==32240)	// Snowy
	{
		        
		if(m_caster->IsFlymount()==false)
		{
			m_caster->SetUInt32Value(147, 17696);
			WorldPacket data2(835, (20));  
			data2 << uint8(0xFF) << m_caster->GetGUID();
	        data2 << uint32(2);
            m_caster->SendMessageToSet(&data2,true);
	...
	// Windrider mounts - epic
	if(m_spellInfo->Id ==32242)	// Swift Blue
	{
		        
		if(m_caster->IsFlymount()==false)
		{
			m_caster->SetUInt32Value(147, 17759);
			WorldPacket data2(835, (20));  
			data2 << uint8(0xFF) << m_caster->GetGUID();
	        data2 << uint32(2);
            m_caster->SendMessageToSet(&data2,true);
	...
	if(m_spellInfo->Id ==32290)	// Swift Green
	{
		        
		if(m_caster->IsFlymount()==false)
		{
			m_caster->SetUInt32Value(147, 17703);
			WorldPacket data2(835, (20));  
			data2 << uint8(0xFF) << m_caster->GetGUID();
	        data2 << uint32(2);
            m_caster->SendMessageToSet(&data2,true);
	...
	if(m_spellInfo->Id ==32289)	// Swift Red
	{
		        
		if(m_caster->IsFlymount()==false)
		{
			m_caster->SetUInt32Value(147, 17718);
			WorldPacket data2(835, (20));  
			data2 << uint8(0xFF) << m_caster->GetGUID();
	        data2 << uint32(2);
            m_caster->SendMessageToSet(&data2,true);
	...
	if(m_spellInfo->Id ==32292)	// Swift Purple
	{
		        
		if(m_caster->IsFlymount()==false)
		{
			m_caster->SetUInt32Value(147, 17717);
			WorldPacket data2(835, (20));  
			data2 << uint8(0xFF) << m_caster->GetGUID();
	        data2 << uint32(2);
            m_caster->SendMessageToSet(&data2,true);
	...
	//fly form 
	if(m_spellInfo->Id ==33943)
	{
		        
		if(m_caster->IsFlymount()==false)
		{
			m_caster->SetUInt32Value(147, 15590);
			WorldPacket data2(835, (20));  
			data2 << uint8(0xFF) << m_caster->GetGUID();
	        data2 << uint32(2);
            m_caster->SendMessageToSet(&data2,true);
			...
			data << uint8(0xFF) << m_caster->GetGUID();
			data << (uint32)0;
			data << (float)25; // left as defualt netherdrake
			m_caster->SendMessageToSet(&data,true);
		}
...
void Spell::SendChannelUpdate(uint32 time)
{
    ...
    WorldPacket data( MSG_CHANNEL_UPDATE, 4+8+1 ); //2.0.8
	data << (uint8) 0xff; //2.0.8
	data << (uint64) m_caster->GetGUID(); //2.0.8
	data << time;
...
void Spell::SendChannelStart(uint32 duration)
{
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        WorldPacket data( MSG_CHANNEL_START, (4+4+8+1) ); //2.0.8
		data << (uint8) 0xff; //2.0.8
		data << (uint64) m_caster->GetGUID(); //2.0.8
		data << m_spellInfo->Id;
        data << duration;
