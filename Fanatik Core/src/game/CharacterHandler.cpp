...
void WorldSession::HandlePlayerLoginOpcode( WorldPacket & recv_data )
{
    ...
    for(int i = 0; i < 128; i++) //2.0.6
        data << uint8(0);

    SendPacket(&data);

	//credit
	const char* text = "|c8f20ff20Welcome to Project Silvermoon.";
	data.Initialize( 829 ,100);
	data << uint32(4);
	data << text;
	SendPacket(&data);
	...
	data.Initialize( 912, 4 );//2.0.6
	data << uint32(0);
	SendPacket( &data );
    
	pCurrChar->SetFlymount(false);
