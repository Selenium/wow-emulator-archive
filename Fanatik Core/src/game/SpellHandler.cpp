...
void WorldSession::HandleGameObjectUseOpcode( WorldPacket & recv_data )
{
    ...
    switch(t)
    {
        ...
        case GAMEOBJECT_TYPE_FLAGSTAND:                     //24
			...
			//ADDED BY CODEBREAKER IF PLAYER CLICK ON FLAG
			//EXECUTE THE FUNCTION EVENTFLAGTAKEN
			if(_player->InBattleGround())
				{
					switch(spellId){
						case 23333:
							BattleGround::EventFlagTaken(HORDE,_player);
						break;
						case 23335:
							BattleGround::EventFlagTaken(ALLIANCE,_player);
						break;
					}
				}
			//END HERE
...
void WorldSession::HandleCancelAuraOpcode( WorldPacket& recvPacket)
{
	...
	//temp hack 2.0.7 for flying mount
	if(spellId == 32345 || (spellId == 32235) || 
		(spellId >= 32239 && spellId <= 32240) || 
		(spellId >= 32242 && spellId <= 32246) || 
		(spellId >= 32289 && spellId <= 32292) || 
		(spellId >= 32295 && spellId <= 32297))
	{
