...
class BattleGround
{
    ...
    public:
		...
		//ADDED BY CODEBREAKER IF IT'S BUGGY IT'S NORMAL
		bool TeamHasFlag(uint32 Team);
		static void EventFlagTaken(uint32 Team,Player const *p);
		static bool SpawnFlagOnPlayerDeath(uint32 Team,float x,float y,float z);
		bool AddFlag(uint32 Team);
		//FINISH ADD BY CODEBREAKER
