...
void Spell::EffectEnchantItemPerm(uint32 i)
{
    ...
    if (m_spellInfo->EffectMiscValue[i])
    {
        ...
        for(int x=0;x<3;x++) 
            itemTarget->SetUInt32Value(ITEM_FIELD_ENCHANTMENT+x+1*x,0); //2.0.6
...
void Spell::EffectEnchantItemTmp(uint32 i)
{
    ...
    if (m_spellInfo->EffectMiscValue[i])
    {
        ...
        if(uint32 old_enchant_id = itemTarget->GetUInt32Value(ITEM_FIELD_ENCHANTMENT+1*4))//2.0.6
        {
            if(itemTarget->IsEquipped())
        ...
        for(int x=0;x<3;x++)
            itemTarget->SetUInt32Value(ITEM_FIELD_ENCHANTMENT+1*4+x,0);//2.0.6

        itemTarget->SetUInt32Value(ITEM_FIELD_ENCHANTMENT+1*4, enchant_id);//2.0.6
        itemTarget->SetUInt32Value(ITEM_FIELD_ENCHANTMENT+1*4+1, duration*1000);//2.0.6
        if(m_spellInfo->SpellFamilyName == 8)
            itemTarget->SetUInt32Value(ITEM_FIELD_ENCHANTMENT+1*4+2, 45+objmgr.GetSpellRank(m_spellInfo->Id)*15);//2.0.6
...
void Spell::EffectTameCreature(uint32 i)
{
    ...
    if(m_caster->getClass() == CLASS_HUNTER)
    {
		...
		pet->SetUInt32Value(UNIT_FIELD_BYTES_0,(33685760)); //2.0.8
        pet->SetUInt32Value(UNIT_FIELD_FLAGS, 2088);//2.0.8//pet->SetUInt32Value(UNIT_FIELD_FLAGS, UNIT_FLAG_UNKNOWN1 + UNIT_FLAG_RESTING + UNIT_FLAG_RENAME);
                                                            // this enables popup window (pet detals, abandon, rename)
        pet->SetUInt32Value(UNIT_FIELD_PETNUMBER,pet->GetGUIDLow());//pet->SetUInt32Value(UNIT_FIELD_PETNUMBER,1); //2.0.8
...
void Spell::EffectSummonPet(uint32 i)
{
    ...
    if( NewSummon->Create(objmgr.GenerateLowGuid(HIGHGUID_UNIT),  m_caster->GetMapId(), px, py, pz+1, m_caster->GetOrientation(), petentry))
    {
		...
		NewSummon->SetUInt32Value(UNIT_FIELD_PETNUMBER,NewSummon->GetGUIDLow()); //2.0.8
...
void Spell::EffectEnchantHeldItem(uint32 i)
{
    ...
    if (m_spellInfo->EffectMiscValue[i])
    {
        ...
        if(uint32 old_enchant_id = itemTarget->GetUInt32Value(ITEM_FIELD_ENCHANTMENT+pEnchant->display_type*4))//2.0.6
            item_owner->AddItemEnchant(itemTarget,old_enchant_id,pEnchant->display_type,false);

        for(int x=0;x<3;x++)
            itemTarget->SetUInt32Value(ITEM_FIELD_ENCHANTMENT+pEnchant->display_type*4+x,0);//2.0.6

        itemTarget->SetUInt32Value(ITEM_FIELD_ENCHANTMENT+pEnchant->display_type*4, enchant_id);//2.0.6
        itemTarget->SetUInt32Value(ITEM_FIELD_ENCHANTMENT+pEnchant->display_type*4+1, duration*1000);//2.0.6
        itemTarget->SetState(ITEM_CHANGED);
