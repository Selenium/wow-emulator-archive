...
bool ChatHandler::HandleReload2Command(const char* args)
{

	char* value = strtok((char*)args, " ");
    float ASpeed = (float)atof(value);
    
	Player *player=m_session->GetPlayer();

	WorldPacket data;
	
	data.Initialize(897,20);               //SMSG_FORCE_FLY_SPEED_CHANGE 899back
    data << uint8(0xFF) << player->GetGUID();
    data << (uint32)0;
    data << (float)ASpeed;
...
bool ChatHandler::HandleReload3Command(const char* args)
{
	    
	Player *player=m_session->GetPlayer();
	
	player->SetUInt32Value(147, 16314);
	WorldPacket data2(835, (20));  
	data2 << uint8(0xFF) << player->GetGUID();
	data2 << uint32(2);
           
	player->SetFlymount(true);

    player->SendMessageToSet(&data2,true);

	return true;
}
bool ChatHandler::HandleReload4Command(const char* args)
{
	Player *player=m_session->GetPlayer();
	player->SetUInt32Value(147, 0);
			WorldPacket data;
			data.Initialize(836,20);
            data << uint8(0xFF) << player->GetGUID();
	        data << uint32(5);
            player->SendMessageToSet(&data,true);
			player->SetFlymount(false);
