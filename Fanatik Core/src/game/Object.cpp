...
void Object::BuildCreateUpdateBlockForPlayer(UpdateData *data, Player *target) const
{
    ...
    switch(m_objectTypeId)
    {
        ...
        case TYPEID_CONTAINER:
            _BuildMovementUpdate( &buf, 0x08, 0x0 ); //2.0.6
            break;
        case TYPEID_UNIT:
            _BuildMovementUpdate( &buf, 0x68, 0x0 ); //2.0.6
            break;
        case TYPEID_PLAYER:
        {
            if( target == this )                            //build for self
            {
                ...
                buf << GetGUID() ;
                buf << m_objectTypeId;
                _BuildMovementUpdate( &buf, 0x61, 0x0 ); //2.0.6
            }
            //build for other player
            else
            {
                _BuildMovementUpdate( &buf, 0x60, 0x0 ); //2.0.6
            }
        }break;
        case TYPEID_CORPSE:
        ...
        case TYPEID_DYNAMICOBJECT:
        {
            if ((GUID_HIPART(GetGUID())==HIGHGUID_PLAYER_CORPSE) || (GUID_HIPART(GetGUID()) == HIGHGUID_TRANSPORT))
                _BuildMovementUpdate( &buf, 0x4a, 0x0 );//2.0.6 maybe 0x4a
            else
                _BuildMovementUpdate( &buf, 0x48, 0x0 );//2.0.6
        }break;
        //case TYPEID_AIGROUP:
...
void Object::_BuildMovementUpdate(ByteBuffer * data, uint8 flags, uint32 flags2 ) const
{
    *data << (uint8)flags;
    if( m_objectTypeId==TYPEID_PLAYER )
    {
		...
		*data << (uint8)0x00 << (uint8)0x00 << (uint8)0x90 << (uint8)0x40;//2.0.6 unk speed
		*data << (uint8)0xdb << (uint8)0x0f << (uint8)0x49 << (uint8)0x40;//2.0.6 unk speed2
    ...
    if( (m_objectTypeId==TYPEID_CORPSE) || (m_objectTypeId==TYPEID_GAMEOBJECT) || (m_objectTypeId==TYPEID_DYNAMICOBJECT))
    {
		...
		*data << GetGUIDHigh();  //2.0.6
    }

    //*data << (uint32)0x1; 2.0.6

	if(( m_objectTypeId==TYPEID_ITEM ) || (m_objectTypeId==TYPEID_CONTAINER))  //temp hack for tbc beta
    {
       
		*data << (uint8)0x0;    //2.0.6
		*data << (uint8)0x0;	//2.0.6
		*data << (uint8)0x0;	//2.0.6
		*data << (uint8)0x40;	//2.0.6
	}
...
void Object::_BuildMovementUpdate(ByteBuffer * data, uint8 flags, uint32 flags2 ) const
{
    *data << (uint8)flags;
       ...
       */
        ...
        *data << (uint32)getMSTime();
        ...
        else
        {
            ...
            *data << ((Player *)this)->GetTransOffsetZ();
            *data << ((Player *)this)->GetTransOffsetO();
			*data << ((Player *)this)->Getpathnode();//2.0.8
        }
		...
		*data << (uint8)0x00 << (uint8)0x00 << (uint8)0x90 << (uint8)0x40;//*data << GetSpeed( MOVE_WALK );  // temp hack tbc beta 5991 4.5
        
		*data << (uint8)0xdb << (uint8)0x0f << (uint8)0x49 << (uint8)0x40;//*data << GetSpeed( MOVE_RUN );   // temp hack tbc beta 5991
		//*data << GUID_LOPART(GetGUID()); //tbc beta 5991
		
    }
    if( m_objectTypeId==TYPEID_UNIT )
    {
        //*data << (uint32)flags2;      //temp hack for tbc beta 
		*data << (uint32)0;
        *data << (uint32)0xB5771D7F;
        *data << ((Unit *)this)->GetPositionX();
        *data << ((Unit *)this)->GetPositionY();
        ...
        *data << ((Creature*)this)->GetSpeed( MOVE_TURN );
        *data << ((Creature*)this)->GetSpeed( MOVE_WALK ); 
        *data << ((Creature*)this)->GetSpeed( MOVE_RUN ); //2.0.6
       ...
       //*data << GetGUID();     //temp hack for tbc beta 
		*data << GetGUIDHigh();
    }
    if( (m_objectTypeId==TYPEID_CORPSE) || (m_objectTypeId==TYPEID_GAMEOBJECT) || (m_objectTypeId==TYPEID_DYNAMICOBJECT))
    {
		...
		*data << ((WorldObject *)this)->GetGUIDHigh();
    }
	...
	if(( m_objectTypeId==TYPEID_ITEM ) || (m_objectTypeId==TYPEID_CONTAINER))  //temp hack for tbc beta
    {
       //*data << GetGUID();     //temp hack for tbc beta 5991
		*data << (uint8)0x0;    //6144
		*data << (uint8)0x0;	//6144
		*data << (uint8)0x0;	//6144
		*data << (uint8)0x40;	//6144
	}
