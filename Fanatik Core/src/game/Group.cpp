...
uint32 Group::RemoveMember(const uint64 &guid, const uint8 &method)
{
    if(m_members.size() > 1)
    {
        ...
        if (player)
        {
            ...
            data.Initialize(SMSG_GROUP_LIST, 17); //2.0.8
            data<<(uint16)0<<(uint8)0<<(uint32)0<<(uint64)0; //2.0.8
            player->GetSession()->SendPacket(&data);
        }
...
void Group::SendInit(WorldSession *session)
{
    ...
    {
                                                            // guess size
        WorldPacket data(SMSG_GROUP_LIST, (2+4+8+8+1+2+m_members.size()*20+18));//2.0.8
        data << (uint8)m_groupType;
        data << (uint8)myFlag;
		data << (uint8)m_groupType; //2.0.8 unk
        int count = 0;
    ...
    for(vector<MemberSlot>::const_iterator citr=m_members.begin(); citr!=m_members.end(); citr++)
    {
        ...
        if(!player || !player->GetSession())
            continue;
                                                            ...
                                                            // guess size
        data.Initialize(SMSG_GROUP_LIST, (6+8+8+1+2+m_members.size()*20+18));
        data << (uint8)m_groupType;
