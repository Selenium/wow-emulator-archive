    ...
    Case RS_LOGON_CHALLENGE, RS_RECONNECT_CHALLENGE
        ...
        If intLoginAccountID <> -1 Then
            Select Case intAccountStatus(intLoginAccountID)
                Case 0
                 Log "Account found [" & LoginName & "]" ' The account is found!
                 Answer_CLIENT_AUTH_LOGON_CHALLENGE sckIndex, intLoginAccountID ' Sending a message back to client
...
Sub UpdateRealms()
'Adding a world server to the realm list
    Dim i As Integer
    'Realms_count = 0
    ...
    frmCore.txtWSName.Text = "vWoW EMU Server"
End Sub
