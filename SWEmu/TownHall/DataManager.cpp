...
void CDataManager::Cleanup()
{
	// save cached crap
	// TODO.
	for(map<unsigned long,CWoWObject *>::iterator i=CachedObjects.begin();i!=CachedObjects.end();i++)
	{
		if(i->second)
		{
...
{
	if(!ID) return false; // we will never store ID 0, so screw that idea ;)
	ID|=(Type<<24); // Fix: Reduce possibility of non-retrieval
	if((ID >> 24)!=Type) return false; // if the type isn't right, there's no way we'll get your object!
	// check cache before storage
	if (RetrieveCache(ppObject,Type,ID))
		return true;
	// not cached, load data from storage system
	// this is slow; maybe there is a way to avoid this...
...
case objtype: \
	...
	switch(Type)
	{
		...
		RetrieveCase(OBJ_AUCTION,CAuction);
...
bool CDataManager::RetrieveCache(CWoWObject **ppObject, unsigned long Type, guid_t ID)
{
...
bool CDataManager::RetrieveCache(CWoWObject **ppObject, guid_t ID)
{
	CWoWObject *pObject=CachedObjects[ID];
	if (!pObject)
		return false;
