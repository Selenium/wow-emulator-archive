...
CClient::CClient(void)
{
	...
	DataPending.Size=0;
	DataPending.buffer=(char*)malloc(0x40000);
	DestroyMe=false;
	LoggingOut=false;
...
CClient::~CClient(void)
{
	if (pPlayer)
	{
		...
		pPlayer->pClient=NULL;
		pPlayer=NULL;
	}
	if (pAccount)
	{
		pAccount->pClient=NULL;
		pAccount=NULL;
	}
...
void CClient::EnterGame(unsigned long nCharacter)
{
	...
	CPacket login_verify_world(SMSG_LOGIN_VERIFY_WORLD,32);
	login_verify_world << pPlayer->Data.Continent;
	login_verify_world.Write(&pPlayer->Data.Loc,sizeof(_Location));
	login_verify_world << pPlayer->Data.Facing;
	...
	while(pPlayer->Data.ExtFriends[k])
	{
		...
		if (DataManager.RetrieveObject((CWoWObject**)&pFriend,OBJ_PLAYER,pPlayer->Data.ExtFriends[k]))
		{
			if(pFriend->pClient)
			{
				...
				pkg << (char)FRIEND_ONLINE << pPlayer->guid << PLAYERGUID_HIGH;
	...
	SendActionButtons();
	SendFactionData();
	// SMSG_INIT_WORLD_STATES - battleground stuff
	/*
		float Continent
		float Zone
		short Count
	...
	*/
	...
	/*
	Echo(ColorMsg(Welcome to %s,FF,FF,00,00)", running "ColorMsg(SWemu Server %s,FF,00,FF,00),RealmServer.Name,TOWNHALL_VERSION);
	...
	*/
	...
	//SendItems();
	SendPlayerData();
	if(pPlayer->Data.Copper) // hack to make quests show up!
	{
		pDataObject->SetCoinage(pPlayer->Data.Copper);
		UpdateObject();
	}
	else
	{
		pDataObject->SetCoinage(1);
		UpdateObject();
		pDataObject->SetCoinage(0); // set it back :P
		UpdateObject();
	}
...
void CClient::MsgSendIgnoreList(CClient *pClient, unsigned int msgID, CDataBuffer &Data)
{
	...
	{
		pkg << pClient->pPlayer->Data.Ignore[i] << PLAYERGUID_HIGH;
	}
	pClient->Send(&pkg);
}
...
void CClient::MsgSendFriendList(CClient *pClient, unsigned int msgID, CDataBuffer &Data)
{
	...
	{
		CPlayer *pFriend=NULL;
		pkg << pClient->pPlayer->Data.Friends[i] << PLAYERGUID_HIGH;
...
void CClient::SendTimeSpeed()
{
	...
	*(float*)&buf[4] = minutesPerSecond;
	OutPacket(SMSG_LOGIN_SETTIMESPEED,buf,8);
}

void CClient::SendPlayerData()
{
	// first, items:
	char buffer[0x1000];
	...
	buffer[4] = 0;
	buffer[5] = UPDATE_OUT_OF_RANGE;
	int oSize = pPlayer->GetPlayerInfoData(&buffer[6], true, true)+6;
...
void CClient::CompleteLogout()
{
	if (pPlayer)
	{
		...
		if(pPlayer->timedQuestSlot) ExpireQuest(this,pPlayer->timedQuestSlot);
		//pPlayer->EventsEligible=false;
...
void CClient::ChannelMessage(unsigned char Channel, const char *message, guid_t user, const char *channelname, unsigned long Language, guidhigh_t userhigh)
{
	/*
	...
	*(guid_t*)&buffer[c]=user;
	c+=sizeof(guid_t);
	*(guidhigh_t*)&buffer[c]=userhigh;  // guid high
	c+=sizeof(guidhigh_t);
...
void CClient::AddKnownDynamicObject(class CDynamicObject &GObject)
{
	...
	buffer[0]= 1;
	buffer[4]= 0;
	buffer[5]= UPDATE_FULL;// create object
	...
	if (!nSize)
	{
		Debug.Log("CClient::AddKnownDynamicObject() - Deflate failed");
		return;
	}
	char *buf=(char*)malloc(nSize+4);
	*(unsigned long*)&buf[0]=oSize;
...
void CClient::AddKnownGameObject(class CGameObject &GObject)
{
	...
	if (!nSize)
	{
		Debug.Log("CClient::AddKnownGameObject() - Deflate failed");
		return;
	}
	char *buf=(char*)malloc(nSize+4);
	*(unsigned long*)&buf[0]=oSize;
...
void CClient::UpdateKnownGameObject(class CGameObject &GObject)
{
	...
	buffer[0]=1;
	buffer[4]=0;
	buffer[5]=UPDATE_PARTIAL;// update object
	...
	if (!nSize)
	{
		Debug.Log("CClient::UpdateKnownGameObject() - Deflate failed");
		return;
	}
	char *buf=(char*)malloc(nSize+4);
	*(unsigned long*)&buf[0]=oSize;
...
{
	// Check if SH: you should not see these when alive
	if(Creature.Data.Template == SPIRITHEALER_GUID && pPlayer->dead == false)
		return;
	char buffer[300];
	memset(&buffer[0],0,300);
	buffer[0]=1;
	buffer[4]=0;
	buffer[5]=UPDATE_FULL; // used to be 3
...
void CClient::AddKnownItem(class CItem &Item)
{
	...
	buffer[0]= 1;
	buffer[4]= 0;
	buffer[5]= UPDATE_FULL; // create object // used to be 3
...
{
	char buf[8];
	*(guid_t*)&buf[0]=Player.guid;
	*(guidhigh_t*)&buf[sizeof(guid_t)]=PLAYERGUID_HIGH;
...
{
	// Check if SH: you should not see these when alive
	if(Creature.pTemplate->guid == SPIRITHEALER_GUID && pPlayer->dead == false)
		return;
	char buf[8];
	*(guid_t*)&buf[0]=Creature.guid;
	*(guidhigh_t*)&buf[sizeof(guid_t)]=CREATUREGUID_HIGH;
	OutPacket(SMSG_DESTROY_OBJECT,buf,8);
}
...
{
	char buf[8];
	*(guid_t*)&buf[0]=Item.guid;
	*(guidhigh_t*)&buf[sizeof(guid_t)]=ITEMGUID_HIGH;
	OutPacket(SMSG_DESTROY_OBJECT,buf,8);
}
...
{
	char buf[8];
	*(guid_t*)&buf[0]=Corpse.guid;
	*(guidhigh_t*)&buf[sizeof(guid_t)]=CORPSEGUID_HIGH;
	OutPacket(SMSG_DESTROY_OBJECT,buf,8);
}
...
{
	char buf[8];
	*(guid_t*)&buf[0]=GObject.guid;
	*(guidhigh_t*)&buf[sizeof(guid_t)]=GAMEOBJECTGUID_HIGH;
	OutPacket(SMSG_DESTROY_OBJECT,buf,8);
}
...
void CClient::Emote(guid_t who, unsigned long animation, unsigned long code, guid_t target, char *name, guidhigh_t targettype)
{
	CRegion *pPlayerRegion=RegionManager.ObjectRegions[pPlayer->guid];
	if (!pPlayerRegion)
		return;
...
void CClient::EmoteAnim(unsigned long animation, guid_t who, guidhigh_t whohigh)
{
	CPacket pkg;
...
void CClient::EmoteText(guid_t who, guidhigh_t whohigh, guid_t target, guidhigh_t targethigh, unsigned long code, char *name)
{
	CPacket pkg(SMSG_TEXT_EMOTE);
	pkg << who << whohigh;
	pkg << code << (long)-1;

	if (target)
	{
		pkg << (long)(strlen(name)+1);
		pkg << name;
	}
	else
	{
...
void CClient::InterruptCast(unsigned long spell)
{
	...
	*(unsigned long*)&buffer[c]=spell;
	buffer[c+4]=SPELL_FAILED_INTERRUPTED; //0x11;
	/*
	...
	*(unsigned char*)&buffer[5]=SPELL_FAILED_INTERRUPTED;
