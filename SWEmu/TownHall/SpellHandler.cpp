...
void CSpellHandler::MsgCastSpell(CClient *pClient, unsigned int msgID, CDataBuffer &Data)
{
	...
	if(pSpell->CastTime==0xFFFFFFFF) pSpell->CastTime=0;
...
{
	if(!pClient->pPlayer) return; // ya wtf
...
void CSpellHandler::MsgUseItem(CClient *pClient, unsigned int msgID, CDataBuffer &Data)
{
	...
	tItem = pClient->pPlayer->Items[slot];
	if(!tItem || !tItem->pTemplateData)
		return;
...
{
	// WARNING: PLEASE CHECK FOR !pObject: some spells do NOT require a target so this will be called with NULL!!
	...
	for(Effect=0;Effect<3;Effect++)
	{
		...
		case SPELL_EFFECT_APPLY_AREA_AURA:
			{
				...
				even->SetEventData(SpellID,pSpell->getPower(SpellID,Effect),EffectID,pSpell->SpellInfo.School,pCaster,NULL,(short)(time/periodicity),periodicity,0xFFFFFFFF,radius,pSpell);
		...
		case SPELL_EFFECT_PERSISTENT_AREA_AURA:
			{
				if(pSpell->SpellInfo.EffectApplyAuraName[Effect]==0x3)
				{
					...
					dO->AddPeriodicAreaDmg(periodicity,(short)(time/periodicity));
		...
		case SPELL_EFFECT_ADDITIONAL_DMG:
			{
				if(!pObject) break; // can't use this without a target...
		...
		case SPELL_EFFECT_ADD_COMBO_POINTS:
			{
				// spell CAN be used w/o a target...
		...
		case SPELL_EFFECT_AURA:
			{
				...
				if(AuraSlot==-1) break; // no more aura slots??
		...
		case SPELL_EFFECT_SUMMON_PET: // should work
			{
				// Summon model
				unsigned long ctguid = DataManager.CreatureTemplates(pSpell->SpellInfo.EffectMiscValue[Effect]);
