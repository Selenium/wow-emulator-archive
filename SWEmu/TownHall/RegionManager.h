...
public:
	...
	{// O(n)
		...
		//if(difftime(now,lastWander) < 0.100f) return true; //every 100ms
		lastWander=now;
		RegionObjectNode *pNode=pList;
		RegionObjectNode *pNext=NULL;
...
public:
	...
	/*hash_*/map<unsigned long,float> HeightMap;
	...
	inline float HeightAt(float X,float Y)
	{
		...
		/*hash_*/map<unsigned long,float>::iterator i;
...
public:
	...
	map<guid_t,CRegion*> ObjectRegions;
	...
	map<unsigned long,CContinent *> Continents;
