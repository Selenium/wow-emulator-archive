...
#if 1 // #ifdef WIN32
// avoid line translation, this *may* happen in certain *nix environments.
...
public:
	...
	bool RetrieveObject(ObjectStorage &Data, guid_t ID)
	{
	...
	bool StoreObject(ObjectStorage &Data, guid_t ID)
	{
		if (!file)
		{
			if (!Safe)
		...
		fflush(file); // force write: players exceed bufsize so this makes sure they don't get fried
		return true;
	}

	bool DeleteObject(guid_t ID)
	{
		if (unsigned long Pos=Positions[ID])
		{
			fseek(file,Pos,SEEK_SET);
	...
	guid_t HighestID;
private:
	map<guid_t,unsigned long> Positions;
...
public:
	...
	bool RetrieveObject(CWoWObject &Object, guid_t ID)
	{
		ObjectStorage Data;
	...
	bool EnumObjects(unsigned long Type, fStorageEnum EnumStorage)
	{
		...
		{
			ObjectStorage Data;
			if (guid_t ID=Files[Type].RetrieveLinear(Data,i))
			{
	...
	bool EnumObjectIDs(unsigned long Type, fObjectIDEnum EnumObjectIDs)
	{
		...
		for (unsigned long i = 0 ; i < Total ; i++)
		{
			if (guid_t ID=Files[Type].RetrieveLinear(i))
			{
				if (!EnumObjectIDs(ID))
					return false;
			}
