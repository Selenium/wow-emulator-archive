...
CGameObject::CGameObject(void):CWoWObject(OBJ_GAMEOBJECT), CUpdateObject(GAMEOBJECT_END)
{
}

bool CGameObject::StoringData(ObjectStorage &Storage)
{
	if (!guid)
		return false;
	Storage.Allocate(sizeof(GameObjectData));
	memcpy(Storage.Data,&Data,sizeof(GameObjectData));
	return true;
}

bool CGameObject::LoadingData(ObjectStorage &Storage)
{
	if (!guid)
		return false;
	memcpy(&Data,Storage.Data,sizeof(GameObjectData));
	DataManager.RetrieveObject((CWoWObject **)&pTemplate,OBJ_GAMEOBJECTTEMPLATE,Data.TemplateID);
	return true;
}
...
void CGameObject::New(guid_t TemplateID)
{
	Clear();
	CWoWObject::New();
	EventsEligible=true;
...
#undef Skip
	...
	CUpdateBlock block(&buffer[c], GAMEOBJECT_END);
