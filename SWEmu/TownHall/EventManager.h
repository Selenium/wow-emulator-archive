...
public:
	...
	inline multimap<UINT64, WoWEvent*>::iterator FindInTimeList(WoWEvent *const RemovedEvent)
	{
		...
		while(itr->second != RemovedEvent)
		{
			...
			{
				Debug.Logf("CEventManager::RemoveEvent() - Cannot remove event #%u", RemovedEvent->EventType);
	...
	{
		return (long)(((Last.time-First.time)*1000)+(Last.millitm-First.millitm));
	}
	...
	static inline void RemoveTime(_timeb &Now, unsigned long ms)
	{
		time_t secs=ms/1000;
		unsigned short msecs=(unsigned short)ms%1000;
		if(Now.millitm<msecs)
		{
			Now.time-=secs+1;
			Now.millitm+=1000-msecs; // -=msecs-1000
		}
		else
		{
			Now.time-=secs;
		...
		/*
		long tmp = Now.millitm;int n=0;long temp=0;
		while(tmp<(long)ms) {tmp+=1000;n++;}
		temp=tmp-ms;
		Now.time-=n;
		Now.millitm=(unsigned short)temp;
		*/
	}
...
#define	EVENT_PLAYER_QUEST_EXPIRE			20
...
#define EVENT_CREATURE_FACETARGET			13
