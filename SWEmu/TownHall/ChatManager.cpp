...
#undef ADDSUBTYPE
	...
	UserLevels["privileged"] = USER_PRIVILEGED;
	...
	Races["draenei"] = RACE_DRAENEI;
	Races["bloodelf"] = RACE_BLOODELF;
}
...
void CmdAura(CClient *pClient, char *input)
{
	if(!input[0])
	{
		pClient->Echo("Syntax: !aura <auraid>");
		return;
	}
	long i1 = atoi(input);
	...
	pClient->pPlayer->AddUpdateVal(UNIT_FIELD_AURAFLAGS+4, 0xd );
	pClient->pPlayer->AddUpdateVal(UNIT_FIELD_AURASTATE, 0x2 );
	pClient->UpdateObject();
}

void CmdMakeWaypoint(CClient *pClient)
{
...
void CmdSaveMe(CClient *pClient)
{
	_timeb now;
	_ftime(&now);
	if(EventManager.DiffTime(now, pClient->pPlayer->LastAttack) < 20000) // No regen for 20 secs after last attack
	{
		...
		pClient->Echo("You cannot be saved while in battle...");
		return;
	}
...
void CmdSendTo(CClient *pClient, char **argv, int argc)
{
	...
	MakeLower(name);
	map<string, guid_t>::iterator i = DataManager.PlayerNames.find(name);
	...
	string str = argv[1];
	MakeLower(str);
	map<string, unsigned long>::iterator race_iter = ChatManager.Races.find(str);
	if(race_iter == ChatManager.Races.end())
	{
		pClient->Echo("Valid Locations are: Human, Dwarf, Gnome, NightElf, Orc, Troll, Tauren, Undead, Draenei, Blood Elf");
		pClient->Echo("Syntax: !sendto <player> <location>");
		return;
	}
	int race=race_iter->second;
	Packets::TeleportOrNewWorld(pPlayer->pClient,RaceStartingPoints[race].Continent,RaceStartingPoints[race].Loc);
...
void CmdDeathDurabilityLoss(CClient *pClient)
{
...
void CmdClients(CClient *pClient)
{
...
void CmdGetMount(CClient *pClient)
{
	...
	{
		pClient->Echo("You must be atleast Level 10 to mount.");
		return;
	}
...
void CmdMount(CClient *pClient, char *input)
{
	...
	if(input[0] == 0)
	{
		...
		pClient->Echo("Syntax: !mount <mount> or !mount <modelid>");
		return;
	}

	unsigned long model=0;
	if(sscanf(input, "0x%X", &model) == 0 && sscanf(input, "%u", &model) == 0)
	{
		string lcase=input;
		MakeLower(lcase);
	...
	pClient->pPlayer->MountedAuraSlot = -1;
	pClient->UpdateObject();
	pClient->pPlayer->SetSpeed(DEFAULT_PLAYER_RUN_SPEED*2);
...
void CmdUnmount(CClient *pClient)
{
...
void CmdDist(CClient *pClient, char *input)
{
	CWoWObject *pTarget=NULL;
	if(!(input[0]))
	{
		pClient->Echo("Syntax:\n\"!distance 2\" for 2-dimensional distance,\n\"!distance 3\" for 3-dimensional distance.");
		return;
	}
	int n=atoi(input);
	if(n==2)
...
{
	if(pClient->pPlayer->Items[SLOT_TABARD] != 0)
	{
	...
	else
	{
		...
		pItem->New(DataManager.ItemTemplates(5976),pClient->pPlayer->guid); // HARDCODED 5976 HERE!
...
void CmdPet(CClient *pClient, char *input)
{	//Pets are not supported anymore
	/*unsigned long Template = 0;
	string templatename;
	if(input[0] == 0)
	{
		pClient->Echo("Syntax: !pet <creature name>");
		return;
	}
	templatename = input;
	...
	CCreature *pCreature;
	unsigned long cont=pClient->pPlayer->Data.Continent;
	_Location loc=pClient->pPlayer->Data.Loc;
	char petname[64];
	sprintf(petname,"%s's bitch",pClient->pPlayer->Data.Name);
	pCreature = new CCreature;
	CCreatureTemplate *pNewTemplate=new CCreatureTemplate;
	pNewTemplate->New(petname);
	if(pTemplate) pNewTemplate->Data=pTemplate->Data;
	strncpy(pNewTemplate->Data.Name,petname,63);
	//pNewTemplate->Data.Level = pClient->pPlayer->Data.Level;
	//pNewTemplate->Data.NormalStats.HitPoints = 28+30*pClient->pPlayer->Data.Level;
	pNewTemplate->Generated=true;
	pNewTemplate->Data.Model = modelid;
	pNewTemplate->Data.Faction = pClient->pPlayer->FFaction;
	DataManager.NewObject(*pNewTemplate);
	pCreature->New(*pNewTemplate);
	pCreature->pTemplate=pNewTemplate;
	pCreature->bIsSummon = true;
	pCreature->summoner = pClient->pPlayer;

	pCreature->Data.Continent=pClient->pPlayer->Data.Continent;
	pCreature->Data.Loc=pClient->pPlayer->Data.Loc;
	pCreature->FirstPoint = 0;

	pCreature->Data.SpawnLoc=pClient->pPlayer->Data.Loc;
	pCreature->Data.SpawnFacing=pClient->pPlayer->Data.Facing;
	pCreature->Data.Facing=pClient->pPlayer->Data.Facing;

	pCreature->Data.Summoner = pClient->pPlayer->guid;
	pCreature->Data.SourceSpell = 0;
	...
	pClient->pPlayer->AddUpdateVal(UNIT_FIELD_SUMMON, pCreature->guid, CREATUREGUID_HIGH);
	pClient->pPlayer->UpdateObject();
	pClient->pPlayer->SummonGuid = pCreature->guid;

	CPacket pkg;
	pkg.Reset(SMSG_PET_SPELLS);
	//Packets::PackGuid(pkg,pCreature->guid,CREATUREGUID_HIGH);
	pkg << pCreature->guid << CREATUREGUID_HIGH;
	pkg << (unsigned long)0x00000000 << (unsigned long)0x00001000
...
void EditCreature(CClient *pClient, CCreature &Creature, char **argv, int argc)
{
	...
	*/
	else if (!strnicmp(argv[0],"size",4))
	{
		...
		{
			Creature.pTemplate->Data.Size=(float)atof(argv[1]);
			Creature.AddUpdateVal(OBJECT_FIELD_SCALE_X, Creature.pTemplate->Data.Size);
	...
	else if (!strnicmp(argv[0],"model",5))
	{
		...
		Creature.pTemplate->Data.Model=val;
		Creature.AddUpdateVal(UNIT_FIELD_DISPLAYID, val);
	...
	*/
	else if (!strnicmp(argv[0],"level",5))
	{
		...
		Creature.pTemplate->Data.Level=atoi(argv[1]);
		Creature.AddUpdateVal(UNIT_FIELD_LEVEL, atoi(argv[1]));
...
void CmdRehash(CClient *pClient)
{
...
void CmdMorph(CClient *pClient, char *input)
{
	if(!input[0])
	{
	...
	unsigned long model=0;
	if(!sscanf(input, "0x%X", &model) && !sscanf(input, "%u", &model))
	{
		pClient->Echo("Invalid model value. Prefix hex values with 0x");
		return;
	}
...
void CmdDeathtouch(CClient *pClient, char *input)
{
	if(input[0])
	{
		string Who=input;
		MakeLower(Who);
		guid_t PlayerID=DataManager.PlayerNames[Who];
		if (!PlayerID)
		{
			pClient->Echo("Player '%s' does not exist.",input);
			return;
		}
		...
		if (DataManager.RetrieveObject((CWoWObject**)&pTarget,OBJ_PLAYER,PlayerID))
		{
			if (!pTarget->pClient)
			{
				pClient->Echo("Player '%s' not online.",input);
				return;
			}
			/*
			...
			*(guid_t*)&packet[0x17]=pTarget->pClient->pPlayer->guid;
			*(guid_t*)&packet[0x22]=pTarget->pClient->pPlayer->guid;
					...
					while(pNode)
					{
						...
						case OBJ_PLAYER:
							...
							if (pPlayer->pClient!=pClient && pPlayer->pClient->pAccount->Data.UserLevel<USER_GM)
							{
								...
								*(guid_t*)&packet[0x17]=pPlayer->guid;
								*(guid_t*)&packet[0x22]=pPlayer->guid;
...
void CmdMaxsent(CClient *pClient)
{
...
void CmdUserLevel(CClient *pClient, char **argv, int argc)
{
	...
	if(name == "admin")
		return;
	map<string, guid_t>::iterator i = DataManager.AccountNames.find(name);
	guid_t AccountID = 0;
	...
	case USER_PRIVILEGED:
		if (pClient->pAccount->Data.UserLevel<USER_GM)
		{
			pClient->Echo("Only GM or greater can assign PRIVILEGED.");
		}
		else
		{
			pAccount->Data.UserLevel=USER_PRIVILEGED;
			if(notifyUser)
				pAccount->pClient->Echo("Your account has been given PRIVILEGED status.");
			pClient->Echo("%s has been given PRIVILEGED status.",pAccount->Data.Name);
		}
		break;
	case USER_NORMAL:
...
void CmdResetPlayer(CClient *pClient, char *input)
{
	if(!input[0])
	{
...
void CmdKickPlayer(CClient *pClient, char *input)
{
	if(!input[0])
	{
...
void CmdGive(CClient *pClient, char **argv, int argc)
{
	if(argc == 2)
	{
		...
		guid_t TemplateID=DataManager.ItemTemplates(ItemID);
		if(!TemplateID)
		{
...
void CmdStrip(CClient *pClient, char *input)
{
	if(!input[0])
	{
	...
	if(i == DataManager.PlayerNames.end())
	{
		pClient->Echo("Unable to find player '%s'.", input);
		return;
	}
	CPlayer *pPlayer = NULL;
	if(!DataManager.RetrieveObject((CWoWObject**)&pPlayer, OBJ_PLAYER, i->second))
	...
	{
		if(CItem *pItem = pPlayer->Items[i])
		{
...
void CmdKillSpawn(CClient *pClient)
{
...
void CmdMorphTarget(CClient *pClient, char **argv, int argc)
{
	if(argc < 2)
	{
		pClient->Echo("Syntax: !morphplayer <model> <playername>");
		pClient->Echo("Use !edit model to change the selected unit's model.");
	}
...
void CmdMountModel(CClient *pClient, char *input)
{
	if(!input[0])
	{
...
void CmdGetPosition(CClient *pClient, char *input)
{
	if(!input[0])
	{
...
void CmdGoto(CClient *pClient, char *input)
{
	if(!input[0])
	{
...
void CmdSummon(CClient *pClient, char *input)
{
	if(!input[0])
	{
...
void CmdRelease(CClient *pClient, char *input)
{
	if(!input[0])
	{
...
void CmdReturn(CClient *pClient, char *input)
{
	if(!input[0])
	{
...
void CmdCape(CClient *pClient, char *input)
{
	map<string, unsigned long>::iterator i;
	if(input[0])
	{
	...
	if(input[0] != 0 || i == ChatManager.Cloaks.end())
	{
	...
	if(pClient->pPlayer->Items[SLOT_BACK] != 0)
...
void CmdShowStats(CClient *pClient, char **argv, int argc)
{
	...
	else if(DataManager.RetrieveObject((CWoWObject**)&pCreature, OBJ_CREATURE, pClient->pPlayer->TargetID))
	{
		...
		if (showtype) pClient->Echo("NPCFlags: %d",pCreature->pTemplate->Data.NPCFlags);
		if (showmodel) pClient->Echo("Model: 0x%X",pCreature->pTemplate->Data.Model);
		if (showloc)
		...
		if (showlevel) pClient->Echo("Level: %d",pCreature->pTemplate->Data.Level);
		//if (showexp) pClient->Echo("Exp: %d",pCreature->pTemplate->Data.Exp);
		if (showhps) pClient->Echo("HP: %d",pCreature->pTemplate->Data.NormalStats.HitPoints);
		if (showdmg) pClient->Echo("DMG: %d to %d",pCreature->pTemplate->Data.DamageMin,pCreature->pTemplate->Data.DamageMax);
		if (showsize) pClient->Echo("Size: %f",pCreature->pTemplate->Data.Size);
		if (showfaction) pClient->Echo("Faction: %d",pCreature->pTemplate->Data.Faction);
	}
	else
	{
		pClient->Echo("You must select a Creature or Player to get info.");
...
void CmdWorldSave(CClient *pClient)
{
	pClient->Echo("Worldsave initiated.");
	WorldSave();
}
void CmdLevel(CClient *pClient, char* input) {
	if(!input[0])
	{
	...
	if(sscanf(input, "0x%X", &level) == 0 && sscanf(input, "%u", &level) == 0)
	{
...
void CmdDisableRegion(CClient *pClient)
{
	CRegion *pRegion = RegionManager.ObjectRegions[pClient->pPlayer->guid];
	if (!pRegion)
	{
...
void CmdTest(CClient *pClient, char** argv, int argc) {
	...
	// Put any testing code here (especially useful for testing packet structures and stuff)
...
void CmdEnableAllQuests(CClient *pClient)
{
	if(pClient->pPlayer->EnableAllQuests)
	{
		pClient->Echo("EnableAllQuests Disabled.");
		pClient->pPlayer->EnableAllQuests=false;
	}
	else
	{
...
void CmdHelpWad(CClient *pClient)
{
...
MsgHandler Handlers[] =
{
	{"!distance", USER_NORMAL, (void*)CmdDist, FUNCTYPE_STRING},
	...
	// {"!clients", USER_NORMAL, (void*)CmdClients, FUNCTYPE_NONE},
	{"!tabard", USER_NORMAL, (void*)CmdTabard, FUNCTYPE_NONE},
	{"!vote", USER_NORMAL, (void*)CmdVote, FUNCTYPE_STRING},
	{"!help", USER_NORMAL, (void*)CmdCommands, FUNCTYPE_NONE},
	{".help",USER_NORMAL, (void*)CmdHelpWad, FUNCTYPE_NONE}, //help for braindead wowemu users xD
	...
	//{"!saveme", USER_NORMAL, (void*)CmdSaveMe, FUNCTYPE_NONE},
	{"!createguild", USER_MODERATOR, (void*)CmdCreateGuild, FUNCTYPE_STRING},
	//{"!getmount", USER_NORMAL, (void*)CmdGetMount, FUNCTYPE_NONE},
	...
	//{"!unmount", USER_NORMAL, (void*)CmdUnmount, FUNCTYPE_NONE}, //alias
	{"!dismount", USER_NORMAL, (void*)CmdUnmount, FUNCTYPE_NONE},
	//{"!morph", USER_MODERATOR, (void*)CmdMorph, FUNCTYPE_STRING},
#ifndef ACCOUNTLESS
	//{"!userlevel", USER_MODERATOR, (void*)CmdUserLevel, FUNCTYPE_ARGS},
#endif
	//{"!cape", USER_MODERATOR, (void*)CmdCape, FUNCTYPE_INPUT},
	//{"!cloak", USER_MODERATOR, (void*)CmdCape, FUNCTYPE_INPUT}, //alias
	{"!showstats", USER_MODERATOR, (void*)CmdShowStats, FUNCTYPE_ARGS},
	{"!goplayer", USER_MODERATOR, (void*)CmdGoto, FUNCTYPE_STRING},
	...
	//{"!getposition", USER_MODERATOR, (void*)CmdGetPosition, FUNCTYPE_STRING},
	{"!spawn", USER_HALFGM, (void*)CmdSpawn, FUNCTYPE_STRING},
	{"!pet", USER_HALFGM, (void*)CmdPet, FUNCTYPE_STRING},
	{"!modify", USER_HALFGM, (void*)CmdEdit, FUNCTYPE_ARGS},
	{"!edit", USER_HALFGM, (void*)CmdEdit, FUNCTYPE_ARGS},
	//{"!level", USER_HALFGM, (void*)CmdLevel, FUNCTYPE_STRING},
	{"!uber", USER_HALFGM, (void*)CmdUber, FUNCTYPE_NONE},
	{"!kill", USER_HALFGM, (void*)CmdKillSpawn, FUNCTYPE_NONE},
	// {"!deathtouch", USER_GM, (void*)CmdDeathtouch, FUNCTYPE_STRING},
	// {"!maxsent", USER_GM, (void*)CmdMaxsent, FUNCTYPE_NONE},
	{"!broadcast", USER_GM, (void*)CmdBroadcast, FUNCTYPE_STRING},
	//{"!resetplayer", USER_GM, (void*)CmdResetPlayer, FUNCTYPE_STRING},
	{"!kickplayer", USER_GM, (void*)CmdKickPlayer, FUNCTYPE_STRING},
	{"!strip", USER_GM, (void*)CmdStrip, FUNCTYPE_STRING},
	{"!morphplayer", USER_GM, (void*)CmdMorphTarget, FUNCTYPE_ARGS},
	//{"!mountmodel", USER_GM, (void*)CmdMountModel, FUNCTYPE_STRING},
	{"!summon", USER_GM, (void*)CmdSummon, FUNCTYPE_STRING},
	//FYI: Summon is to force "naughty" players to come to you and get a spanking ;) They should be kept frozen during this.
	{"!release", USER_GM, (void*)CmdRelease, FUNCTYPE_STRING},
	{"!return", USER_GM, (void*)CmdReturn, FUNCTYPE_STRING},
	{"!callvote", USER_GM, (void*)CmdCallVote, FUNCTYPE_STRING},
	{"!tally", USER_GM, (void*)CmdTally, FUNCTYPE_NONE},
	...
	//{"!rehash", USER_ADMIN, (void*)CmdRehash, FUNCTYPE_NONE},
	...
	{"!aura", USER_ADMIN, (void*)CmdAura, FUNCTYPE_STRING},
	{"!makewaypoint", USER_ADMIN, (void*)CmdMakeWaypoint, FUNCTYPE_NONE},
	{"!addscriptcommand", USER_ADMIN, (void*)CmdAddScriptCommand, FUNCTYPE_STRING},
	{"!worldsave",USER_HALFGM,(void*)CmdWorldSave,FUNCTYPE_NONE},
	{NULL, 0, 0},
};
...
void CChatManager::MsgChat(CClient *pClient, unsigned int msgID, CDataBuffer &Data)
{
	...
	case CHAT_SAY:
		if(!ProcessCommand(pClient, &Data.Buffer()[8]))
			pClient->ChatSay(0/**(unsigned long*)&Data.Buffer()[4]*/,&Data.Buffer()[8]);
		break;
	case CHAT_PARTY:
		{
	...
	case CHAT_OFFICER:
		...
		break;
	case CHAT_YELL:
		pClient->ChatYell(0/**(unsigned long*)&Data.Buffer()[4]*/,&Data.Buffer()[8]);
		break;
	case CHAT_CHANNEL:
		{
	...
	case CHAT_WHISPER:
		{
			unsigned long Language=0/**(unsigned long*)&Data.Buffer()[4]*/;
			...
			{
				if(pTo->Data.Ignore[count]==pClient->pPlayer->guid)
				{
...
void CChannel::SetMode(CPlayer *p, const char *p2n, bool mod, bool set)
{
	...
	{
		CPlayer *newp = NULL;
		guid_t myguid = DataManager.PlayerNames[p2n];
		if(!myguid)
		{
			MakeNotOn(&data,p2n);
			SendToOne(&data,p);
...
void CChannel::SetOwner(CPlayer *p, const char *newname)
{
	...
	{
		CPlayer *newp = NULL;
		guid_t myguid = DataManager.PlayerNames[newname];
		if(!myguid)
		{
...
void CChannel::Say(CPlayer *p, const char *what)
{
	...
	else
	{
		...
		data << messageLength;
		data << what;
		if(p->Data.StatusFlags & STATUS_GM) data << (unsigned char)0x03;
		else if(p->Data.StatusFlags & STATUS_AFK) data << (unsigned char)0x01;
		else data << (unsigned char)0x00;
