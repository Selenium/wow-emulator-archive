...
#if TBC
#define FACTION_ARRAY_COUNT 128 // TBC new: 128 factions
#else
...
struct PlayerData
{
	...
	// using FACTION_ARRAY_COUNT effectively prevents TBC builds from using 1.12 data and viceversa
	// that's probably better to prevent any issues...
...
//	unsigned long Items[120];// note: theres only 69 total items currently, but we'll keep this 120 in case of expansions etc
	CItem* Items[LAST_SLOT_PLAYER];// note: theres only 69 total items currently, but we'll keep this 120 in case of expansions etc
...
//	unsigned long BagItems[120];	// mmm 20 slots per bag x 4 bags this should be more than enough
	...
	bool IsFacing(CWoWObject *pObject);
	...
	inline void name(long val) \
		...
		inline void SetMountModel(unsigned long model)
		{
			if(model != pObject->Data.MountModel)
			{
				...
				if(model)
					pObject->AddUpdateVal(UNIT_FIELD_FLAGS, UNIT_FLAG_MOUNT | UNIT_FLAG_MOUNT_ICON | UNIT_FLAG_SHEATHE);
				else
					pObject->AddUpdateVal(UNIT_FIELD_FLAGS, UNIT_FLAG_SHEATHE | 0x1008);
		...
		inline void SetRestState(unsigned char newState)
		{
			pObject->Data.RestState = newState;
			pObject->AddUpdateVal(PLAYER_BYTES_2, (pObject->Data.RestState << 24) | (0xEE00) | (pObject->Data.Appearance[4]));
		};

		inline void SetPvPRank(unsigned char newRank)
		{
		...
		inline void SetDrunkState(unsigned char newDrunkenness)
		{
...
/*
		inline void SetItem(int slot, unsigned long guid)
		{
			...
			if (slot < 19)
			{
				...
				{
					CItem *pItem = NULL;
					if (DataManager.RetrieveObject((CWoWObject**)&pItem, pObject->Items[slot]))
		...
		{
			guid_t ItemGuid = pItem ? pItem->guid : 0;
			pObject->Items[slot] = pItem;
			...
			if (pItem && (pItem->Data.Owner != pObject->guid)){
				pItem->Data.Container = pObject->guid;
				pItem->AddUpdateVal(ITEM_FIELD_CONTAINED,pObject->guid,ITEMGUID_HIGH);
				pItem->UpdateItem(pItem->guid, pObject);
			}
		}
/*
		inline void SetBagItem(const int slot, CItem *const pItem)
		{
			...
			pObject->Items[slot] = pItem;
			pObject->AddUpdateVal(CONTAINER_FIELD_SLOT_1 + slot*2, ItemGuid, ItemGuid ? ITEMGUID_HIGH : 0);
		}
*/

	inline void SetBagItem(const int slot, const int bag_slot, CItem *const pItem)
	{
		const guid_t ItemGuid = pItem ? pItem->guid : 0;
		CItem *pBag = pObject->Items[bag_slot];
		if (!pBag) return;
		pObject->Items[GetBagItemSlot(bag_slot, slot)] = pItem;
		if (pItem) {
			pItem->Data.Container = pBag->guid;
			pItem->AddUpdateVal(ITEM_FIELD_CONTAINED,pBag->guid,ITEMGUID_HIGH);
			pItem->UpdateItem(ItemGuid, pObject);
		}
		pBag->AddUpdateVal(CONTAINER_FIELD_SLOT_1 + (slot*2), ItemGuid,ITEMGUID_HIGH);
		pBag->UpdateItem(pBag->guid, pObject);
	}


		static inline unsigned short GetBagItemSlot(unsigned char bag, unsigned char bagSlot){
...
#undef ADDFLOAT
	...
	unsigned char FindBag(CItem *const pItem){
		for (int i = 0; i < 4 ;i++){
			if (Items[SLOT_BAG1 + i] == pItem) return i;
		}
		return 0xFF;
	}
};
