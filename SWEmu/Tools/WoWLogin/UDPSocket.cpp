...
int UDPSocket::SendTo(const char *buf,  int len, Addr &to)
{
	if (s==INVALID_SOCKET)
		return 0;
	int ret= sendto(s,buf,len,0,(sockaddr*)&to,sizeof(Addr));
	return ret;
}

int UDPSocket::RecvFrom(char * buf, int len, Addr &from)
{
	if (s==INVALID_SOCKET)
		return 0;
	int fromlen=sizeof(sockaddr);
	int ret = recvfrom(s,buf,len,0,(sockaddr*)&from,(socklen_t*)&fromlen);
	return ret;
}
