...
bool TCPSocket::Send(char csend)
{
	if (m_hSocket==INVALID_SOCKET)
		return false;
	return (send(m_hSocket,&csend,sizeof(char),0)==1);
}
...
int TCPSocket::Send(const char *csend, int length)
{
	if (m_hSocket==INVALID_SOCKET)
		return 0;
	int ret = send(m_hSocket,(char*)csend,length,0);
	if (ret==SOCKET_ERROR)
		return 0;
	return ret;
}
...
int TCPSocket::Receive(char *crecv, int length, int flags)
{
	if (m_hSocket==INVALID_SOCKET)
		return 0;
	int ret = recv(m_hSocket,crecv,length,flags);
	if (ret==SOCKET_ERROR)
		return 0;
	return ret;
}
...
char *TCPSocket::ErrorName(int error)
{
	...
	case WSAEWOULDBLOCK:
		return "WSAEWOULDBLCK (10035): Resource temporarily unavailable";
	}
	return "Unknown";
}
/**/
