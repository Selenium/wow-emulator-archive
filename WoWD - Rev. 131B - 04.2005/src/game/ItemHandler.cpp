...
void WorldSession::HandleSwapInvItemOpcode( WorldPacket & recv_data )
{
    ...
    {
        if (!srcitem || !GetPlayer()->CanEquipItemInSlot(dstslot, srcitem->GetProto()->InventoryType))
        {
...
void WorldSession::HandleBuyItemInSlotOpcode( WorldPacket & recv_data ) // drag & drop
{
    ...
    {
        char buf[256];
        sprintf( (char*)buf, "SMSG_BUY_FAILED: Not enough Coins" );
...
void WorldSession::HandleBuyItemOpcode( WorldPacket & recv_data ) // right-click on item
{
    ...
    {
        char buf[256];
        sprintf( (char*)buf, "SMSG_BUY_FAILED: Inventory Full" );
        sChatHandler.FillSystemMessageData(&data, this, buf);
        SendPacket( &data );
        return;
    }
...
void WorldSession::HandleListInventoryOpcode( WorldPacket & recv_data )
{
    ...
    {
        curItem = objmgr.GetItemPrototype(unit->getItemId(itemcount));
        if( !curItem )
        {
            Log::getSingleton( ).outError( "Unit %i has nonexistant item %i!", guid, unit->getItemId(itemcount) );
        ...
        else
        {
            ...
            data << uint32( unit->getItemId(itemcount) ); // item id
            data << uint32( curItem->DisplayInfoID ); // item icon
            data << uint32( unit->getItemAmount(itemcount) ); // number of items available, -1 works for infinity, although maybe just 'cause it's really big
    ...
    WPAssert(data.size() == 8 + 1 + numitems * 7 * 4);
    SendPacket( &data );
