...
Module BigInt
	...
	'
	Public Const Fx As Integer = 2147483629
	Public Const WrkD As String = "c:\temp\" ' set path working directory
	Public Const MD As Short = 10000
	Public Const M2 As Integer = 99990000
	Public Const MB As Integer = &H10000 'dec, bin storage bases
	Public F() As Integer
	Public l() As Short
	Public sg() As Short
	...
	Sub Add(ByRef i0 As Short, ByRef i1 As Short)
		sg(i1) = -sg(i1)
		Subt(i0, i1) : sg(i1) = -sg(i1)
	End Sub
	
	Sub Binf(ByRef i0 As Short)
		Dim c2, C, c0, cs As Integer
		Dim K, j, ll As Short
		Dim q, S As Short
		Dim r As Boolean
		ll = l(i0) - 1
	...
	Function Cmp(ByRef i0 As Short, ByRef i1 As Short) As Short
		Dim S, j, X As Short
		X = sg(i0)
		If X = sg(i1) Then
			S = l(i0) - l(i1)
	...
	Function Cvl(ByRef G As String) As Integer
		Dim C As Integer
		Dim K As Short
		K = Len(G) : C = 0
		If K < 4 Then G = G & New String(Chr(0), 4 - K)
		For K = 4 To 1 Step -1
			C = C * 256 + Asc(Mid(G, K, 1))
		Next K : Cvl = C
	End Function
	
	Sub Copyf(ByRef i0 As Short, ByRef i1 As Short)
		Dim j As Short
		For j = 0 To l(i0)
			F(i1, j) = F(i0, j)
		Next j
	...
	Sub Dcr(ByRef i0 As Short, ByRef A As Short)
		Dim r As Short
		A = System.Math.Abs(A)
		r = F(i0, 0) - sg(i0) * A
	...
	Sub Divd(ByRef i0 As Short, ByRef i1 As Short, ByRef i2 As Short)
		Dim c0, C, cs As Integer
		Dim j As Short
		Dim l1, K, l0, l2 As Short
		Dim m, ll, q As Short
		Dim nr As Double
		If Isf(i1, 0) Then
			j = MsgBox("div. by zero in Sub Divd", 277, "Error")
			If j = 2 Then FileClose() : End '        [Cancel]
	...
	Sub Euclid(ByRef i0 As Short, ByRef i1 As Short, ByRef i2 As Short)
		Dim S As Short
	...
	Sub Fctl(ByRef i0 As Short, ByRef N As Short)
		Dim C As Integer
		Dim j, lx As Short
		Dim nr As Double
		Dim r As Short
		...
		Do  : nr = r
			...
			End If
			Mult(i0, t1, t2)
			System.Windows.Forms.Application.DoEvents() : If Key Then Exit Sub
		Loop Until r > N
	End Sub
	
	Sub Inc(ByRef i0 As Short, ByRef A As Short)
		Dim r As Short
		A = System.Math.Abs(A)
		r = F(i0, 0) + sg(i0) * A
		If r > 0 And r < MD Then
	...
	Function Isf(ByRef i0 As Short, ByRef A As Short) As Boolean
		Dim r, S As Short
		S = System.Math.Sign(A) : If A = 0 Then S = sg(i0)
		r = F(i0, 1) + F(i0, 0) = System.Math.Abs(A)
		Isf = r And (sg(i0) = S) And (l(i0) = 1)
	End Function
	
	Function Ismin1(ByRef i0 As Short, ByRef i1 As Short) As Boolean
		Ismin1 = 0
		If l(i0) > l(i1) And sg(i0) = 1 Then Exit Function
		Inc(i0, 1) : Ismin1 = Cmp(i0, i1) = 0 : Dcr(i0, 1)
	End Function
	
	Function IsPPrm(ByRef i0 As Short, ByRef t3 As Short, ByRef t4 As Short, ByRef w As Short) As Boolean
		Dim j, A, df, K As Short
		Dim Sw, X As Boolean
		Dim r As Short
		X = -1
		...
		If Not Sw Then '                      try small divisors
			...
			For r = 1 To K
				...
				End If
				System.Windows.Forms.Application.DoEvents() : If Key Then Exit Function
			Next r
		End If
		'
		...
		K = 0 : Do  '                         determine N - 1 = (2^ k) * odd(m)
			K = K + 1 : Rsft(t3, 1)
		Loop Until -CShort(F(t3, 0) And 1)
		Binf(t3) : Randomize(VB.Timer())
...
jmp: 
	...
	Sub Isqrt(ByRef i0 As Short, ByRef i1 As Short)
		Dim C As Integer
		Dim l1, j, ll As Short
		Dim r, tr As Single
		Dim S As Short
		If sg(i0) = -1 Then
		...
		Loop Until System.Math.Abs(tr - r) < 1
		l1 = ll \ 2 : l(i1) = l1 + 1
		For j = 0 To l(i1) : F(i1, j) = 0 : Next 
		S = CShort(ll And 1) * 99 : sg(i1) = 1 '   compose seed x0 in i1
		F(i1, l1) = CInt(r * (1 + S))
		'
	...
	Function IsSqr(ByRef i0 As Short, ByRef i1 As Short) As Boolean
		'Square test from Henri Cohen's `Computational Number Theory', 1.7.2
		Dim C As Integer
		Dim K, k2 As Short
		Static q11(10) As Short
		Static q63(62) As Short
		Static q64(63) As Short
		Static q65(64) As Short
	...
	Function Kronec(ByRef i0 As Short, ByRef i1 As Short, ByRef i2 As Short) As Short
		Dim S, r, X As Short
		X = 1
		...
		If (F(t1, 0) And 1) = 0 Then
			...
			r = 0 : Do 
				r = r + 1 : Rsft(t1, 1)
			Loop Until -CShort(F(t1, 0) And 1)
			If -CShort(r And 1) Then
				r = F(t0, 0) And 7
		...
		Do 
			...
			If (F(t0, 0) And 1) = 0 Then
				...
				r = 0 : Do 
					r = r + 1 : Rsft(t0, 1)
				Loop Until -CShort(F(t0, 0) And 1)
				If -CShort(r And 1) Then
					r = F(t1, 0) And 7
					If r = 3 Or r = 5 Then X = -X
				End If
...
jump: 
	...
	Function Les(ByRef G As String) As Short
		G = Trim(G)
		Les = (1 + (Len(G) - 1) \ 4) * 2
	End Function
	
	Sub Letf(ByRef i0 As Short, ByRef A As Short)
		F(i0, 1) = 0 : F(i0, 0) = System.Math.Abs(A) '   abs(a) < MD
		l(i0) = 1 : sg(i0) = System.Math.Sign(A + 0.5) '  SGN(0) = 1
	End Sub
	
	Sub LetfD(ByRef i0 As Short, ByRef C As Integer, ByRef cM As Integer)
		Dim cs As Integer
		Dim j As Short
		sg(i0) = System.Math.Sign(C + 0.5) : cs = System.Math.Abs(C)
		j = 0 : Do 
	...
	Sub Lsft(ByRef i0 As Short, ByRef r As Short)
		Dim C, c0 As Integer
		Dim j As Short
		Dim l0, q As Short
		Select Case r
	...
	Function Mkl(ByRef C As Integer) As String
		Dim ct As Integer
		Dim S As String
		S = "" : ct = C : Do 
			S = S & Chr(ct And 255)
			ct = ct \ 256
		Loop Until ct = 0
		Mkl = S & New String(Chr(0), 4 - Len(S))
	End Function
	
	Sub Moddiv(ByRef i0 As Short, ByRef i1 As Short)
		Dim S As Short
		Divd(i0, i1, t1)
	...
	Sub Modmlt(ByRef i0 As Short, ByRef i1 As Short, ByRef i2 As Short)
		Mult(i0, i1, t1)
		Divd(i0, i2, t1)
	End Sub
	
	Sub Modpwr(ByRef i0 As Short, ByRef i1 As Short, ByRef i2 As Short)
		Dim c0, C, c2 As Integer
		Dim j As Short
		Dim K, S As Short
		Dim Sw As Boolean
	...
	Sub Modsqu(ByRef i0 As Short, ByRef i1 As Short)
		Squ(i0, t1) : Swp(i0, t1)
		Divd(i0, i1, t1)
	End Sub
	
	Sub Mult(ByRef i0 As Short, ByRef i1 As Short, ByRef i2 As Short)
		Dim c0, C, c2 As Integer
		Dim j, K As Short
		Dim l2, l0, l1, ll As Short
		Dim lx, m As Short
		Dim Sw As Boolean
	...
	Function Nxtprm(ByRef Sw As Boolean) As Integer
		Dim b4 As New VB6.FixedLengthString(4)
		Dim r As Short
		Static C, cp, cpl, cb As Integer
		Static dc, i As Short
				...
				Do  : dc = 6 - dc : C = C + dc
					If i = 30 Then
						...
						If cp < cpl Then '            next bitvector in PrimFlgs.bin
							...
							FileGet(2, b4.Value, cp)
							cb = Cvl(b4.Value) : cp = cp + 4
						Else
							cb = 469171647
						End If
					End If
	...
	Sub Printf(ByRef i0 As Short, ByRef G As String, ByRef h As String, ByRef Sw As Boolean)
		Dim j, ll As Short
		Dim S As String
	...
	Sub Pwr10(ByRef i0 As Short, ByRef K As Short)
		Dim r, j, S As Short
		sg(i0) = 1 : K = System.Math.Abs(K)
		l(i0) = 1 + K \ 4
	...
	Sub Ratdec(ByRef i0 As Short, ByRef i1 As Short, ByRef G As String, ByRef lt As Short)
		Dim K, j, lx As Short
		Dim S As String
		Divd(i0, i1, t1)
		If Isf(i0, 0) Then
	...
	Sub Readst(ByRef i0 As Short, ByRef G As String)
		Dim l0, K, ll As Short
		G = Trim(G) : sg(i0) = 1
		If G = "" Then G = " "
		If Left(G, 1) = "-" Then
			G = Mid(G, 2) : sg(i0) = -1
		End If
		l0 = Len(G) : ll = 4 - CShort(l0 And 3)
		If ll < 4 Then
			G = New String(" ", ll) & G : l0 = l0 + ll
		End If
		ll = l0 - 3 : l0 = l0 \ 4
	...
	Sub Rsft(ByRef i0 As Short, ByRef r As Short)
		Dim C, c0 As Integer
		Dim j As Short
		Dim l0, q As Short
		Select Case r
	...
	Sub Squ(ByRef i0 As Short, ByRef i1 As Short)
		Dim C, c0 As Integer
		Dim j As Short
		Dim l0, K, l1 As Short
		Dim lx, ll, m As Short
		l0 = l(i0) : lx = l0 + l0 - 1
		If lx >= UBound(F, 2) Then
	...
	Sub Subt(ByRef i0 As Short, ByRef i1 As Short)
		Dim ix, im, j As Short
		Dim lx, lm, S As Short
	...
	Sub Swp(ByRef i0 As Short, ByRef i1 As Short)
		Dim r As Short
		r = i0 : i0 = i1 : i1 = r
	End Sub
	
	
	Public Sub HexToBigInt(ByRef Tekst As String, ByRef bigint As Short)
	...
	Public Function BigIntToHex(ByVal bigint As Short) As String
		Dim i As Short
		Dim A As Short
		Dim B As Short
		Dim C As Short
	...
	Public Function ModExp(ByRef base As Short, ByRef exp As Short, ByRef modulus As Short, ByRef result As Short) As Object
		'  Modular exponentiation: base i0, binary exponent i1, modulus i2,
		'  the result is in i0. Modulo reduction is disabled if i2 is empty.
