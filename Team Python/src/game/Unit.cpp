...
void Unit::Update( uint32 p_time ) 
{
    ...
    if (m_state & UF_TARGET_DIED)
	{
        ...
        uint32 new_health = getUpdateValue(UNIT_FIELD_MAXHEALTH)*.8;
		if (new_health > getUpdateValue(UNIT_FIELD_HEALTH))
...
//            SendMessageToSet(&data, false);
			world.SendUnitAreaMessage(&data, WorldServer::getSingletonPtr()->GetCreature(m_guid[0]));
...
//            printf("Removing corpse...\n");
		    std::list<Unit*>::iterator itr; //erase mobs attack list
...
//                    if ((world.GetItem(randItem)->Class == 2) || (world.GetItem(randItem)->Class == 4)) {
					if (world.GetItem(randItem)->Sellprice)
                        found = 1;
//                    }
                }
            }
...
void Unit::AI_Update()
{
    ...
    for (itr = m_attackers.begin(); itr != m_attackers.end(); ++itr)
    {
		...
		{
			if (m_Pet == false) AI_MoveTo(respawn_cord[0][0], respawn_cord[0][1], respawn_cord[0][2], true); //LINA, force mobs to return to there respawn
	        uint32 max_health = getUpdateValue(UNIT_FIELD_MAXHEALTH)*.8;
		    setUpdateValue(UNIT_FIELD_HEALTH, max_health);
