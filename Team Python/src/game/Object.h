...
public:
    ...
    inline bool getUpdateMaskBit( const uint16 &index ) const {
        return (m_updateMask[ index >> 3 ] & ( 1 << ( index & 0x7 ) ))!=0;
    }
    inline uint32 getUpdateValue( const uint16 &index ) const { return m_updateValues[ index ]; }
    inline float getUpdateFloatValue( const uint16 &index ) const { return m_updateFloats[ index ]; }
