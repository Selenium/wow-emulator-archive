...
//Taxi charges Money (taken from SiTWulf)
//balko start
                DatabaseInterface *dbi = Database::getSingleton( ).createDatabaseInterface( );
                uint32 path = dbi->getPath( sourcenode, destinationnode );
				uint32 price = dbi->getTaxiPrice( sourcenode, destinationnode );
...
//balko start	
					...
					ChatHandler pChat;
					pChat.FillMessageData(&data, 0x09, pClient, (uint8*)"You don't have enought money to travel");
					pClient->SendMsg(&data);
				}
//balko end			
			}break;    
