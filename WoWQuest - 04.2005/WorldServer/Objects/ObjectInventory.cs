...
namespace WoWDaemon.World
{
	...
	public class PlayerInventory : ObjectInventory
	{
		...
		{
			for(int i = (int)INVSLOT.BACKPACK_FIRST;i < (int)INVSLOT.BACKPACK_LAST;i++)//NUM_INVENTORY_SLOTS
				if(m_slots[i] == 0)
					return (byte)i;
			return (byte)0;
		}
		...
		public void ItemMove(int fromSlot, int toSlot)
		{
			...
			m_owner.UpdateValue(((int)PLAYERFIELDS.INV_SLOTS)+fromSlot*2);
			m_owner.UpdateValue(((int)PLAYERFIELDS.INV_SLOTS)+toSlot*2);
		...
		{
			m_slots[slot] = 0;
			m_owner.UpdateValue(((int)PLAYERFIELDS.INV_SLOTS)+slot*2);
			if(slot < 19)
			{
		...
		public override void PreCreateOwner(bool isClient)
		{
			...
			if(isClient)
			{
				for(int i = (int)INVSLOT.NONE_EQUIPFIRST;i < (int)INVSLOT.PLAYER_INVENTORY_SLOTS;i++)//NUM_INVENTORY_SLOTS
				{
					if(m_slots[i] != 0)
					{
						Owner.UpdateValue(((int)PLAYERFIELDS.INV_SLOTS)+i*2);
