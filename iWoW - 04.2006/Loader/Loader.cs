...
//				address = Dns.GetHostEntry(Dns.GetHostName()).AddressList[0];
				address = IPAddress.Parse("192.168.3.100");
...
//				address = Dns.Resolve(Dns.GetHostName()).AddressList[0];
		...
		static bool ParseWorldServers(XmlNodeList worldServers)
		{
			...
			foreach(XmlNode node in worldServers)
			{
				...
				{
					Console.Write("Could not start world server: {0}\n{1}\n", e.Message, e.StackTrace);
		...
		{
			Console.WriteLine("Welcome to iWoW.");
		...
		Console.WriteLine("Done");
			...
			Console.WriteLine("Loginserver started on " + LoginServer.Instance.LocalEndPoint);
			Console.WriteLine("Redirectserver started on " + RedirectServer.Instance.LocalEndPoint);
			if(localRealmListServerStarted)
		...
		private static void RestartLoop() {
			while(true) {
				...
				if (LoginServer.RestartServer) {
			        ...
			        {
				        Console.WriteLine("Loader:Unhandled exception!");
				        Console.WriteLine(e.Message);
				        Console.WriteLine(e.StackTrace);
					} finally {
						DebugLogger.Logger.Log("Restarted the server...");
