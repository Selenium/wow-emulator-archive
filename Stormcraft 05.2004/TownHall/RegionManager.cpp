...
CContinent::CContinent(float xTop, float xBottom, float xLeft, float xRight)
{
...
// left 3725
// right -6400
	Continents[0]=new CContinent(5200.0f,-16000.0f,3700.f,-6400.0f);
...
// left 3725
// right -8500
	Continents[1]=new CContinent(11700.0f,-12700.0f,3700.f,-8500.0f);
}
...
{// O(1) for this function, O(n) for each fading region
	...
	// an adjacent region
	switch(pOldRegion->IsAdjacent(*pNewRegion))
	{
	case ADJ_TOPLEFT:
		{
...
void CRegionManager::ObjectNew(CWoWObject &Object, unsigned long Continent, float NewX, float NewY)
{
	...
	if (Continent>=2)
		return;//... fuck you.
	CRegion *pNewRegion=Continents[Continent]->RegionByLoc(NewX,NewY);
	pNewRegion->AddObject(Object);
	ObjectRegions[Object.guid]=pNewRegion;
	for (int i = 0 ; i < 3 ; i++)
		for (int j = 0 ; j < 3 ; j++)
...
bool CRegionManager::GetNearestPathGroup(CPathGroup **ppGroup, unsigned long Continent, _Location &Loc)
{
	...
	if (Continent>=2)
		return false;
	CRegion *pRegion=Continents[Continent]->RegionByLoc(Loc.X,Loc.Y);
	// do we have a list?
