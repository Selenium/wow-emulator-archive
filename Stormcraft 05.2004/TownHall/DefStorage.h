...
public:
	...
	unsigned long GenerateCRC(ObjectStorage &Data)
	{
		return 0;
	}
	unsigned long GenerateCRC(ObjectStorage &Data, unsigned long Size)
	{
		return 0;
	}
...
public:
	...
	bool Handle_Setting(char const * setting, char const * value)
	{
		return false;
	}
	...
	bool EnumObjects(unsigned long Type, fStorageEnum EnumStorage)
	{
		...
		{
			ObjectStorage Data;
			if (unsigned long ID=Files[OBJ_ACCOUNT].RetrieveLinear(Data,i))
			{
	...
	bool EnumObjectIDs(unsigned long Type, fObjectIDEnum EnumObjectIDs)
	{
		...
		for (unsigned long i = 0 ; i < Total ; i++)
		{
			if (unsigned long ID=Files[OBJ_ACCOUNT].RetrieveLinear(i))
			{
				if (!EnumObjectIDs(ID))
					return false;
			}		
