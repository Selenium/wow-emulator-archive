...
#Region "Global.Constants"
    ...
    '2.00.10 - 6448
    '2.00.12 - 6546
    Const REQUIRED_BUILD_LOW As Integer = 6299
    Const REQUIRED_BUILD_HIGH As Integer = 6546
...
#Region "Global.Config"
    ...
    <XmlRoot(ElementName:="RealmServer")> _
        ...
        <XmlElement(ElementName:="SQLDBName")> Public SQLDBName As String = "adb"
        <XmlElement(ElementName:="SQLDBType")> Public SQLDBType As SQL.DB_Type = SQL.DB_Type.MySQL
        <XmlElement(ElementName:="AutoAccountCreate")> Public AutoAccountCreate As Boolean = True
    End Class
    ...
    Public Sub LoadConfig()
        Try
            ...
            'oStmW = New StreamWriter(System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly.Location()) + "\\config.xml")
            'oXS.Serialize(oStmW, Config)
            'oStmW.Close()
            'Loading
            Console.Write("...")
            Dim oStmR As StreamReader
            oStmR = New StreamReader(System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly.Location()) + "\\vbWoW.RealmServer.xml")
...
#Region "RS.Analyzer"
    ...
    Class ClientClass
        ...
        Public Sub Process()
            ...
            ConsoleColor.SetConsoleColor()
            If Not Database.QuerySQL("SELECT ip FROM adb_bans WHERE ip = """ & IP.ToString & """;") Then
...
#Region "RS_OPCODES"
    Public Sub On_RS_LOGON_CHALLENGE(ByRef data() As Byte, ByRef Client As ClientClass)
        ...
        Console.WriteLine("[{0}] [{1}:{2}] CMD_AUTH_LOGON_CHALLENGE [{3}] [{4}], WoW Version [{5}.{6}.{7}.{8}].", Format(TimeOfDay, "hh:mm:ss"), Client.IP, Client.Port, packet_account, packet_ip, bMajor.ToString, bMinor.ToString, bRevision.ToString, ClientBuild.ToString)
        ...
        If ((ClientBuild) >= REQUIRED_BUILD_LOW) And ((ClientBuild) <= REQUIRED_BUILD_HIGH) Then
            ...
            Try
                'Get Account info
                Database.Query([String].Format("SELECT * FROM adb_accounts WHERE account = ""{0}"";", packet_account), result)
                ...
                Else
                    ...
                    If Config.AutoAccountCreate Then
                        'Insert the new account
                        Database.InsertSQL([String].Format("INSERT INTO adb_accounts (account, password, email, joindate, last_ip) VALUES ('{0}', '{1}', '{2}', '{3}', '{4}')", packet_account.ToUpper, packet_account.ToUpper, ("noone@wow.com").ToUpper, Format(Now, "yyyy-MM-dd"), Client.IP.ToString))
                        'Check if the Insert was set.
                        If Database.QuerySQL("SELECT * FROM adb_accounts WHERE account = "" & packet_account & "";") Then acc_state = AccountState.LOGIN_OK
                    End If
                End If
            ...
            Select Case acc_state
                Case AccountState.LOGIN_OK
                    ...
                    Dim PassString As String = result.Rows(0).Item("password")
                    Dim password(PassString.Length - 1) As Byte
                    ...
                    Array.Copy(Client.AuthEngine.unk3, 0, data_response, 102, 16)
                    data_response(118) = 0
    ...
    Public Sub On_RS_LOGON_PROOF(ByRef data() As Byte, ByRef Client As ClientClass)
        ...
        If pass_check Then
            ...
            data_response(24) = 0
            data_response(25) = 0
            If Client.Expansion = ExpansionLevel.NORMAL Then
                data_response(26) = 0
                data_response(27) = 0
            Else
                data_response(26) = &HCC
                data_response(27) = &HCC
            End If
            ...
            For i = 0 To Client.AuthEngine.SS_Hash.Length - 1
                ...
                End If
            Next
            Database.Update([String].Format("UPDATE adb_accounts SET last_sshash = '{1}', last_ip='{2}', last_login='{3}' WHERE account = '{0}';", Client.Account, sshash, Client.IP.ToString, Format(Now, "yyyy-MM-dd")))
        Else
            'Wrong pass
    ...
    Public Sub On_RS_REALMLIST(ByRef data() As Byte, ByRef Client As ClientClass)
        ...
        Dim result As DataTable
        Database.Query([String].Format("SELECT * FROM adb_realms;"), result)

        For Each Row As System.Data.DataRow In result.Rows
            packet_len = packet_len + Len(Row.Item("ws_host")) + Len(Row.Item("ws_name")) + 1 + Len(Format(Row.Item("ws_port"), "0")) + 12
        Next
        ...
        '(byte) unknown1, 0x2A
        '(byte) unknown2, 0x00
        Converter.ToBytes(CType(1, Short), data_response, tmp) '2=list of realms 0=wizard
        Client.Send(data_response)
...
#End Region
    ...
    Sub WS_Status_Report()
        Dim result1 As DataTable
        Database.Query([String].Format("SELECT * FROM adb_realms;"), result1)
        Dim result2 As DataTable
        Database.Query([String].Format("SELECT * FROM adb_realms WHERE ws_status < 2;"), result2)
        Console.WriteLine()
    ...
    End Sub
    Sub Main()
        ConsoleColor.SetConsoleTitle(String.Format("{0} v{1}", [Assembly].GetExecutingAssembly().GetCustomAttributes(GetType(System.Reflection.AssemblyTitleAttribute), False)(0).Title, [Assembly].GetExecutingAssembly().GetName().Version))
        ...
        ConsoleColor.SetConsoleColor(ConsoleColor.ForegroundColors.BrightWhite)
                ...
                If CommandList(varList).Length > 0 Then
                    Select Case cmd(0).ToLower
                        ...
                        Case "createaccount", "/createaccount"
                            Database.InsertSQL([String].Format("INSERT INTO adb_accounts (account, password, email, joindate, last_ip) VALUES ('{0}', '{1}', '{2}', '{3}', '{4}')", cmd(1).ToUpper, cmd(2).ToUpper, cmd(3).ToUpper, Format(Now, "yyyy-MM-dd"), "0.0.0.0"))
                            If Database.QuerySQL("SELECT * FROM adb_accounts WHERE account = "" & packet_account & "";") Then
                                ConsoleColor.SetConsoleColor(ConsoleColor.ForegroundColors.Green)
                                Console.WriteLine("[Account: " & cmd(1).ToUpper & " Password: " & cmd(2).ToUpper & " Email: " & cmd(3).ToUpper & "] Has been created.")
                                ConsoleColor.SetConsoleColor()
                            Else
                                ConsoleColor.SetConsoleColor(ConsoleColor.ForegroundColors.Red)
                                Console.WriteLine("[Account: " & cmd(1).ToUpper & " Password: " & cmd(2).ToUpper & " Email: " & cmd(3).ToUpper & "] could not be created.")
                                ConsoleColor.SetConsoleColor()
                            End If
                        Case Else
                            'Command not found
