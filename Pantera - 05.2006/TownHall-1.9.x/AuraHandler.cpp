...
void AuraHandler::ModForm(CWoWObject *pTarget,int id,bool apply)
{
	...
	case FORM_TREE:
		{
			//do nothin cause this shit is never used :p
			break;
		}
	case FORM_TRAVEL:
		{
	...
	case FORM_STEALTH:
		{
			//i havent touched this it might be buggy
			CPacket pkg;
			((CPlayer*)pTarget)->AddUpdateVal(UNIT_FIELD_BYTES_1,apply? 0x21E0000: 0);
			if (apply == false)
			{
...
long AuraHandler::ApplyAura(CWoWObject *pPlayer, unsigned long SpellID, unsigned long AuraTime,bool positive)
{
	...
	if(pPlayer->type == OBJ_CREATURE) AuraSlot = ((CCreature*)pPlayer)->FindFreeAuraSlot(positive);
	for(int l=0;l<64;l++)
	{
		if (pPlayer->type == OBJ_PLAYER)
		{ 
...
void AuraHandler::ApplyModifier(unsigned long auraslot, unsigned long ModID, unsigned long SpellID, bool apply, unsigned long Effect,long power, unsigned long time, unsigned long type,CWoWObject *pCaster,CWoWObject *pPlayer)
{
	...
	mod.pTarget = pPlayer;
	mod.pCaster = pCaster;
	if(apply) if(pPlayer->type==OBJ_PLAYER) ((CPlayer*)pPlayer)->AddModifier(mod);
	...
	case SPELL_AURA_HOVER: {
		Debug.Log("ApplyModifier: Unsupported SPELL_AURA_HOVER");
		break; }
	...
	case SPELL_AURA_MOD_TOTAL_STAT_PERCENTAGE: {
		...
		{
		case 0:{
			if(power>0) index2 = PLAYER_FIELD_POSSTAT0;
			else index2 = PLAYER_FIELD_NEGSTAT0;
			((CPlayer*)pPlayer)->AddUpdateVal(index2,v);
			((CPlayer*)pPlayer)->AddUpdateVal(UNIT_FIELD_STR,((CPlayer*)pPlayer)->Data.CurrentStats.Strength);
			((CPlayer*)pPlayer)->UpdateObject();
			break; }
		case 1:{
			power ? index2 = PLAYER_FIELD_POSSTAT1 : index2 = PLAYER_FIELD_NEGSTAT1;
			((CPlayer*)pPlayer)->AddUpdateVal(index2,v);
			((CPlayer*)pPlayer)->AddUpdateVal(UNIT_FIELD_AGILITY,((CPlayer*)pPlayer)->Data.CurrentStats.Agility);
			((CPlayer*)pPlayer)->UpdateObject();
			break; }
		case 2:{
			power ? index2 = PLAYER_FIELD_POSSTAT2 : index2 = PLAYER_FIELD_NEGSTAT2;
			((CPlayer*)pPlayer)->AddUpdateVal(index2,v);
			((CPlayer*)pPlayer)->AddUpdateVal(UNIT_FIELD_STAMINA,((CPlayer*)pPlayer)->Data.CurrentStats.Stamina);
			((CPlayer*)pPlayer)->UpdateObject();
			break; }
		case 3:{
			power ? index2 = PLAYER_FIELD_POSSTAT3 : index2 = PLAYER_FIELD_NEGSTAT3;
			((CPlayer*)pPlayer)->AddUpdateVal(index2,v);
			((CPlayer*)pPlayer)->AddUpdateVal(UNIT_FIELD_IQ,((CPlayer*)pPlayer)->Data.CurrentStats.Intellect);
			((CPlayer*)pPlayer)->UpdateObject();
			break; }
		case 4:{
			power ? index2 = PLAYER_FIELD_POSSTAT4 : index2 = PLAYER_FIELD_NEGSTAT4;
			((CPlayer*)pPlayer)->AddUpdateVal(index2,v);
			((CPlayer*)pPlayer)->AddUpdateVal(UNIT_FIELD_SPIRIT,((CPlayer*)pPlayer)->Data.CurrentStats.Spirit);
			((CPlayer*)pPlayer)->UpdateObject();
			break; }
		default:{
			break;
	...
	case SPELL_AURA_SAFE_FALL: {
		apply ? pkg.Reset(SMSG_MOVE_FEATHER_FALL) : pkg.Reset(SMSG_MOVE_NORMAL_FALL);
		pkg << (unsigned char)0x0F << pPlayer->guid;
		((CPlayer*)pPlayer)->pClient->Send(&pkg);
		break; }
	...
	case SPELL_AURA_MOD_RESISTANCE: {
		...
		{
		case 1: {
			if(power>0) index2 = PLAYER_FIELD_RESISTANCEBUFFMODSPOSITIVE;
			else index2 = PLAYER_FIELD_RESISTANCEBUFFMODSNEGATIVE;
			((CPlayer*)pPlayer)->AddUpdateVal(index2,v);
			((CPlayer*)pPlayer)->AddUpdateVal(UNIT_FIELD_RESISTANCES,((CPlayer*)pPlayer)->Data.CurrentStats.Armor);
			((CPlayer*)pPlayer)->UpdateObject();
			break; }
		case DMG_FIRE: {
			if(power>0) index2 = PLAYER_FIELD_RESISTANCEBUFFMODSPOSITIVE+DMG_FIRE;
			else index2 = PLAYER_FIELD_RESISTANCEBUFFMODSNEGATIVE+DMG_FIRE;
			((CPlayer*)pPlayer)->AddUpdateVal(index2,v);
			((CPlayer*)pPlayer)->AddUpdateVal(UNIT_FIELD_RESISTANCES+DMG_FIRE,((CPlayer*)pPlayer)->Data.CurrentStats.ResistFire);
			((CPlayer*)pPlayer)->UpdateObject();
			break; }
		case DMG_NATURE: {
			if(power>0) index2 = PLAYER_FIELD_RESISTANCEBUFFMODSPOSITIVE+DMG_NATURE;
			else index2 = PLAYER_FIELD_RESISTANCEBUFFMODSNEGATIVE+DMG_NATURE;
			((CPlayer*)pPlayer)->AddUpdateVal(index2,v);
			((CPlayer*)pPlayer)->AddUpdateVal(UNIT_FIELD_RESISTANCES+DMG_NATURE,((CPlayer*)pPlayer)->Data.CurrentStats.ResistNature);
			((CPlayer*)pPlayer)->UpdateObject();
			break; }
		case DMG_FROST: {
			if(power>0) index2 = PLAYER_FIELD_RESISTANCEBUFFMODSPOSITIVE+DMG_FROST;
			else index2 = PLAYER_FIELD_RESISTANCEBUFFMODSNEGATIVE+DMG_FROST;
			((CPlayer*)pPlayer)->AddUpdateVal(index2,v);
			((CPlayer*)pPlayer)->AddUpdateVal(UNIT_FIELD_RESISTANCES+DMG_FROST,((CPlayer*)pPlayer)->Data.CurrentStats.ResistNature);
			((CPlayer*)pPlayer)->UpdateObject();
			break; }
		case DMG_SHADOW:{
			if(power>0) index2 = PLAYER_FIELD_RESISTANCEBUFFMODSPOSITIVE+DMG_SHADOW;
			else index2 = PLAYER_FIELD_RESISTANCEBUFFMODSNEGATIVE+DMG_SHADOW;
			((CPlayer*)pPlayer)->AddUpdateVal(index2,v);
			((CPlayer*)pPlayer)->AddUpdateVal(UNIT_FIELD_RESISTANCES+DMG_SHADOW,((CPlayer*)pPlayer)->Data.CurrentStats.ResistNature);
			((CPlayer*)pPlayer)->UpdateObject();
			break; }
		case DMG_ARCANE:{
			if(power>0) index2 = PLAYER_FIELD_RESISTANCEBUFFMODSPOSITIVE+DMG_ARCANE;
			else index2 = PLAYER_FIELD_RESISTANCEBUFFMODSNEGATIVE+DMG_ARCANE;
			((CPlayer*)pPlayer)->AddUpdateVal(index2,v);
			((CPlayer*)pPlayer)->AddUpdateVal(UNIT_FIELD_RESISTANCES+DMG_ARCANE,((CPlayer*)pPlayer)->Data.CurrentStats.ResistNature);
			((CPlayer*)pPlayer)->UpdateObject();
			break; }
		}
	...
	case SPELL_AURA_MOD_ROOT: {
		...
		Packets::PackGuid(pkg,pPlayer->guid,0x00000000000);
	...
	case SPELL_AURA_MOD_STAT: {
		...
		{
			if(power>0)
				v = ModStat(pPlayer,power,spellinfo.EffectMiscValue[Effect],true);
			else v = ModStat(pPlayer,power,spellinfo.EffectMiscValue[Effect],false);
		}
		else 
		{
			if(power>0)
				v = ModStat(pPlayer,-(long)power,spellinfo.EffectMiscValue[Effect],true);
			else v = ModStat(pPlayer,-(long)power,spellinfo.EffectMiscValue[Effect],false);
		}
		switch(spellinfo.EffectMiscValue[Effect])
		{
		case 0:{
	...
	case SPELL_AURA_PERIODIC_DAMAGE: {
		if(apply)
		{
			...
			even->SetEventData(SpellID,power,SPELL_AURA_PERIODIC_DAMAGE,spellinfo.School,pCaster,pPlayer,time/periodicity,periodicity,auraslot,0,0);			
	...
	case SPELL_AURA_MOD_DECREASE_SPEED: {
		if(apply)
		{
			...
			{
				((CCreature*)pPlayer)->MovingSpeed-=(float)(((float)power/100.0f)*((CCreature*)pPlayer)->AI_CreatureMoveSpeed);
	...
	case SPELL_AURA_MOUNTED: {
		...
		{
			((CPlayer*)pPlayer)->MountedAuraSlot = 0;
	...
	case SPELL_AURA_PERIODIC_HEAL: {
		if(apply)
		{
			long periodicity =spellinfo.EffectAmplitude[Effect];
			AuraEvent *even = new AuraEvent();
			even->SetEventData(SpellID,power,SPELL_AURA_PERIODIC_HEAL,spellinfo.School,pCaster,pPlayer,time/periodicity,periodicity,auraslot,0,0);	
	...
	case SPELL_AURA_MOD_POWER_REGEN: {
		Debug.Log("ApplyModifier: Unsupported SPELL_AURA_MOD_POWER_REGEN");
		break; }
	...
	case SPELL_AURA_MOD_ATTACK_POWER: {
		if(apply)
		{
			...
			((CPlayer*)pPlayer)->AddUpdateVal(UNIT_FIELD_ATTACKPOWER,((CPlayer*)pPlayer)->AttackPower()+power);
			((CPlayer*)pPlayer)->UpdateObject();
		}
		else
		{
