...
CDynamicObject::CDynamicObject(void):CWoWObject(OBJ_DYNAMICOBJECT),CUpdateObject(NUM_DYNAMICOBJECT_FIELDS)
{
	Clear();
	CWoWObject::New();
	EventsEligible = true;
...
unsigned long CDynamicObject::AddCreateObjectData(char *buffer)
{
...
#define Fill(size,value) memset(&buffer[c],value,size);c+=size;
	...
	Add(unsigned long, GAMEOBJECTGUID_HIGH);
	*/
	Add(unsigned char,0x06);
	Add(unsigned char,0x50);
	Add(_Location,Data.loc);
	Add(float,0x0);
	Add(unsigned long, 0x6282A48C);

#undef Fill
#undef Add
#undef Skip

	CUpdateBlock block(&buffer[c], NUM_DYNAMICOBJECT_FIELDS);
	...
	case DYNOBJ_AREA_DOT:
		{
			...
			for(int i=0;i<200;i++)
			{
				if(TargetMap[i]&&!(TargetMap[i]->dead))
				{
					...
					{ ULONG power = spell->getPower(Data.SpellID,Data.Effect);
			...
			if(elapsed==Data.count+1) {((CPlayer*)caster)->StopChannel(Data.SpellID);Remove();}
		}
		break;
	}
}
