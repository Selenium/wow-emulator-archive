...
void CGuild::MsgGuildQuery(CClient *pClient, unsigned int msgID, CDataBuffer &Data)
{
	...
	unsigned long id;
	Data >> id;
	if(!DataManager.RetrieveObject((CWoWObject**)&pGuild, id))
	{
...
void CGuild::GuildChannel(CClient *pClient, char *msg)
{
	...
	*(unsigned long*)&buffer[c]=0;
	c+=4;
	//Packets::PackGuidBuffer(&buffer[c],pClient->pPlayer->guid,0);
	...
	for(i = Members.begin();i != Members.end();i++)
	{
		...
		if(pPlayer->pClient != NULL)
		{
			...
			if(pPlayer->Data.Items[SLOT_TABARD] != 0)
			{
				...
				//pPlayer->UpdateObject();
				pPlayer->AddUpdateVal(PLAYER_FIELD_INV_SLOT_HEAD+SLOT_TABARD*2, pPlayer->Data.Items[SLOT_TABARD]->guid, ITEMGUID_HIGH);
			}
