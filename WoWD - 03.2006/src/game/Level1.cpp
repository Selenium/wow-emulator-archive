...
bool ChatHandler::HandleRecallCommand(const char* args)
{
	...
	do
	{
		...
		uint8 loclen = fields[2].GetUInt8();
		uint32 locmap = fields[3].GetUInt32();
		float x = fields[4].GetFloat();
		float y = fields[5].GetFloat();
		float z = fields[6].GetFloat();

		if (strncmp((char*)args,locname,loclen)==0)
		{
			smsg_NewWorld(m_session,locmap,x,y,z);
			delete result;
			return true;
		}
...
bool ChatHandler::HandleModifyASpedCommand(const char* args)
{
	...
	chr->SetPlayerSpeed(RUN, ASpeed, true);

	chr->SetPlayerSpeed(SWIM, ASpeed, true);

	chr->SetPlayerSpeed(RUNBACK, ASpeed, true);
...
bool ChatHandler::HandleModifySpeedCommand(const char* args)
{
    ...
    chr->SetPlayerSpeed(RUN, Speed, true);
...
bool ChatHandler::HandleModifySwimCommand(const char* args)
{
	...
	chr->SetPlayerSpeed(SWIM, Swim, true);
...
bool ChatHandler::HandleModifyBWalkCommand(const char* args)
{
	...
	chr->SetPlayerSpeed(RUNBACK, BSpeed, true);
...
bool ChatHandler::HandleModifyMountCommand(const char* args)
{
	...
	chr->SetFlag( UNIT_FIELD_FLAGS , U_FIELD_FLAG_MOUNT_SIT );

	chr->SetPlayerSpeed(RUN, speed, true);

	chr->SetPlayerSpeed(SWIM, speed, true);
...
bool ChatHandler::HandleModifyValueCommand(const char* args)
{
    ...
    uint16 field = atoi(pField);
    uint32 value   = atol(pValue);

    if (field < 1 || field >= PLAYER_END)
    {
        FillSystemMessageData(&data, m_session, "Incorrect Field.");
        m_session->SendPacket( &data );
        return true;
    }
/*
...
*/
    char buf[256];
	char buf2[256];
	uint32 oldValue = chr->GetUInt32Value(field);
    chr->SetUInt32Value(field,value);

    sprintf((char*)buf,"Set Field %u from %u to %u.", field, oldValue, value);
	FillSystemMessageData(&data, m_session, buf);
	m_session->SendPacket( &data );

	data.clear();
	sprintf((char*)buf2,"Hex: from 0x%.8X to 0x%.8X.",oldValue, value);
    FillSystemMessageData(&data, m_session, buf2);
    m_session->SendPacket( &data );
