...
public:
	...
	uint32 GetActAmt() const { return m_actamt; }
	void SetActAmt(uint32 amt) { m_actamt = amt; }
...
//!
//! Affect *af = new Affect( AFF_GIANTSTR, spellId, 30, caster->GetGUID(), caster)
//!
...
public:
    ...
    Affect(SpellEntry *proto, int32 duration, const uint64 &guid, Unit *caster) :
        m_spellProto(proto), m_duration(duration), m_casterGUID(guid), m_caster(caster),m_damagePerTick(0), m_auraSlot(0), m_spellPerTick(0), m_coAffect(0) {}
    ...
    void SetPeriodicTriggerSpell(uint32 spellId, uint32 tick, uint16 damage)
    {
	...
	void SetLeechPerTick(uint16 dmg, uint32 tick, uint16 type)
	{
		m_leechPerTick = dmg;
        m_tickLeechInterval = tick;
        m_currentLeechTick = tick;
		m_leechType = type;
	}

    void SetEnergizePerTick(uint16 dmg, uint32 tick, uint16 type)
    {
		m_energizePerTick = dmg;
        m_tickEnergizeInterval = tick;
        m_currentEnergizeTick = tick;
		m_energizeType = type;
    }
    ...
    uint32 GetSpellPerTick() const { return m_spellPerTick; }
   	uint32 GetCurrentSpellTick() const { return m_currentSpellTick; }
	uint32 GetLeechPerTick() const { return m_leechPerTick; }
	uint16 GetLeechType() const { return m_leechType; }
    uint32 GetEnergizePerTick() const { return m_energizePerTick; }
    uint16 GetEnergizeType() const { return m_energizeType; }
	uint32 GetUInt32GBValue(int slot) { return m_uint32GBValues[slot]; }
	void SetUInt32GBValue(int slot, uint32 value) { m_uint32GBValues[slot] = value; }
	float GetFloatGBValue(int slot) { return m_floatGBValues[slot]; }
	void SetFloatGBValue(int slot, float value) { m_floatGBValues[slot] = value; }
    ...
    uint8 Update(uint32 diff)
    {
        ...
        if (m_spellPerTick)
        {
            if( m_currentSpellTick <= diff )
            {
                ...
                returnV |= FLAG_PERIODIC_TRIGGER_SPELL;
            }
            else
		...
		if (m_leechPerTick)
		{
			if( m_currentLeechTick <= diff )
			{
				m_currentLeechTick = m_tickLeechInterval;
				returnV |= FLAG_PERIODIC_LEECH;
			}
			else
				m_currentLeechTick -= diff;
		}
		if (m_energizePerTick)
		{
			if( m_currentEnergizeTick <= diff )
			{
				m_currentEnergizeTick = m_tickEnergizeInterval;
				returnV |= FLAG_PERIODIC_ENERGIZE;
			}
			else
				m_currentEnergizeTick -= diff;
		}
        return returnV;
    }
...
private:
	...
	uint16 m_leechPerTick;
    uint32 m_tickLeechInterval;
    uint32 m_currentLeechTick;
	uint16 m_leechType;

	uint16 m_energizePerTick;
    uint32 m_tickEnergizeInterval;
    uint32 m_currentEnergizeTick;
	uint16 m_energizeType;

    Affect* m_coAffect;

    bool m_positive;

	//General Base Values for modifiers that need to store base values
    union 
    {
	    uint32 m_uint32GBValues[2];
        float m_floatGBValues[2];
    };
