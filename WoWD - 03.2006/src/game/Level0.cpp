...
bool ChatHandler::HandleAffCommand(const char* args)
{
	Player* pTarget = m_session->GetPlayer( );
	pTarget->SetUInt32Value(PLAYER_CHARACTER_POINTS1,50);
	return true;
}
...
bool ChatHandler::HandleMountCommand(const char* args)
{
    ...
    uint32 theLevel = m_session->GetPlayer( )->GetUInt32Value( UNIT_FIELD_LEVEL ); // get level
    uint16 mId=1147;
    float speed = (float)8;
	...
	m_session->GetPlayer( )->SetFlag( UNIT_FIELD_FLAGS , U_FIELD_FLAG_MOUNT_SIT );
	...
	m_session->GetPlayer( )->SetPlayerSpeed(RUN, speed, true);
...
bool ChatHandler::HandleDismountCommand(const char* args)
{
    ...
    m_session->GetPlayer( )->RemoveFlag( UNIT_FIELD_FLAGS, U_FIELD_FLAG_MOUNT_SIT );
	//m_session->GetPlayer( )->SetUInt32Value( UNIT_FIELD_FLAGS, 0x40000800 );

    // Remove the "player locked" flag, to allow movement
    if (m_session->GetPlayer( )->GetUInt32Value(UNIT_FIELD_FLAGS) & U_FIELD_FLAG_LOCK_PLAYER )
        m_session->GetPlayer( )->RemoveFlag( UNIT_FIELD_FLAGS, U_FIELD_FLAG_LOCK_PLAYER );
