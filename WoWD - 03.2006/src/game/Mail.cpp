...
void WorldSession::HandleSendMail(WorldPacket & recv_data )
{
	...
	Player* pl = GetPlayer();
	uint32 flags = 0;
	if (strcmp(reciever.c_str(),pl->GetName()) == 0)
		flags = 2;
	uint64 gd = objmgr.GetPlayerGUIDByName(reciever.c_str());
	if (!gd)
		flags = 4;
	...
	if (flags == 0)
	{
		...
		delinvq << "DELETE FROM mail WHERE mailID = " << mID; // TODO: use full guids
...
void WorldSession::HandleMailDelete(WorldPacket & recv_data )
{
	...
	std::stringstream delinvq;
	delinvq << "DELETE FROM mail WHERE mailID = " << message; // TODO: use full guids
	sDatabase.Execute( delinvq.str().c_str( ) );
...
void WorldSession::HandleItemTextQuery(WorldPacket & recv_data )
{
	...
	{
		// See if we have an item text by that guid
		uint32 pageid = mailguid;
		char query[200];
		while(pageid)
		{
