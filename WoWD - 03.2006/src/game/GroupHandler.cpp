...
void WorldSession::HandleGroupInviteOpcode( WorldPacket & recv_data )
{
    ...
    Raid * raid = objmgr.GetRaidByLeader( GetPlayer()->GetGroupLeader() );
    if ( raid != NULL )
    {
        if ( raid->IsFull() )
        {
...
void WorldSession::HandleGroupAcceptOpcode( WorldPacket & recv_data )
{
    ...
    if ( player->IsInGroup() && (player->GetGroupLeader() == player->GetGUID()) )
    {
        ...
        {
            Raid *raid = objmgr.GetRaidByLeader( GetPlayer()->GetGroupLeader() );
            raid->AddMember( GetPlayer()->GetGUID(), GetPlayer()->GetName() );
            raid->SendUpdate();
            return;
        }
...
void WorldSession::HandleGroupUninviteOpcode( WorldPacket & recv_data )
{
    ...
    if(group==NULL)
    {
        ...
        if (raid->RemoveMember(player->GetGUID()) < 1)
        {
            ...
            raid->SendNullUpdate(GetPlayer()->GetGUID());
...
void WorldSession::HandleGroupSetLeaderOpcode( WorldPacket & recv_data )
{
    ...
    if(!group)
    {
        ...
        raid->ChangeLeader( player->GetGUID() );
        return;
    }
...
void WorldSession::HandleGroupDisbandOpcode( WorldPacket & recv_data )
{
    ...
    if(group==NULL)
    {
        ...
        if (raid->RemoveMember(GetPlayer()->GetGUID()) > 1)
            raid->SendUpdate();
        else
        {
            raid->Disband();
...
void WorldSession::HandleLootMethodOpcode( WorldPacket & recv_data )
{
    ...
    if (group == NULL)
    {
        ...
        raid->SetLootMethod( lootMethod );
        raid->SetLooterGuid( lootMaster );
        raid->SendUpdate();
        return;
    }
