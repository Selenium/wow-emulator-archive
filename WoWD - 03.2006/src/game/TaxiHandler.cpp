...
void WorldSession::HandleTaxiNodeStatusQueryOpcode( WorldPacket & recv_data )
{
    ...
    curloc = objmgr.GetNearestTaxiNode( GetPlayer( )->GetPositionX( ), GetPlayer( )->GetPositionY( ), GetPlayer( )->GetPositionZ( ), GetPlayer( )->GetMapId( ) );
...
void WorldSession::HandleActivateTaxiOpcode( WorldPacket & recv_data )
{
    ...
    GetPlayer( )->SetFlag( UNIT_FIELD_FLAGS, U_FIELD_FLAG_MOUNT_SIT );
    ...
    //WPAssert( data.size() == 37 + pathnodes.getLength( ) * 4 * 3 ); //no use cuz of newguid
