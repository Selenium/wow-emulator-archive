...
bool AuthSocket::_HandleLogonChallenge()
{
	...
	sb << "SELECT banned,banTime,countToBan FROM `ipbantable` where ip='" << ipb << "'";
	QueryResult *result = sDatabase.Query( sb.str().c_str() );
	if(result)
    {
        Field *fields = result->Fetch();
        uint8 banned = fields[0].GetUInt8();
        uint32 timeo = fields[1].GetUInt32();
        uint16 countToBan = fields[2].GetUInt16();
        time_t timen;
        timen = time(NULL);
        if(banned == 1 || (timen < timeo))
        {
            sLog.outDebug("Banned ip try to login!");
            delete result;
            //need some drop connection code here TcpSocket::?
            return false;
        }
        if(countToBan == 3 && timen > timeo)
        {
            sb.rdbuf()->str("");
            sb << "DELETE FROM `ipbantable` WHERE `ip` = '" << ipb << "'";
            sDatabase.Execute( sb.str().c_str() );
        }
        delete result;
    }//Finish    
    ...
    else
    {
		...
		ss << "SELECT password, gm, banned FROM accounts WHERE login='" << _login << "'"; // password,banned
...
bool AuthSocket::_HandleLogonProof()
{
    ...
    else
    {
        ...
        uint16 countToBan;
        ipb=GetRemoteAddress().c_str();
		sb << "SELECT countToBan FROM `ipbantable` where ip='" << ipb << "'";
		QueryResult *result = sDatabase.Query( sb.str().c_str() );
		if(result)
		{
            Field *fields = result->Fetch();
            countToBan = fields[0].GetUInt16();
            if(countToBan < 3)
            {
                countToBan++;
                if(countToBan == 2)
                {
                    ...
                    timer = timer+(15*60);
                    sb.rdbuf()->str("");
                    sb << "UPDATE `ipbantable` SET `banTime` = " << (long)timer << " WHERE `ip` = '" << ipb << "'";
                    sDatabase.Execute( sb.str().c_str() );
                }
                sb.rdbuf()->str("");
                sb << "UPDATE `ipbantable` SET `countToBan` = " << countToBan << " WHERE `ip` = '" << ipb << "'";
                sDatabase.Execute( sb.str().c_str() );                
            }
            delete result;
        }
        ...
        {
            sb.rdbuf()->str("");
            sb << "INSERT INTO `ipbantable` VALUES ('" << ipb << "', 0, 0, 0)";
            sDatabase.Execute( sb.str().c_str() );
        }
...
bool AuthSocket::_HandleRealmList()
{
    ...
    std::string ipb;
    ipb=GetRemoteAddress().c_str();
    ss << "DELETE FROM ipbantable WHERE ip='" << ipb << "'";
    sDatabase.Execute( ss.str().c_str() );
